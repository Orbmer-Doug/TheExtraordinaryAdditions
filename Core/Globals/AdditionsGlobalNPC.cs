...
public class AdditionsGlobalNPC : GlobalNPC
{
    ...
    public override void ModifyHitByProjectile(NPC npc, Projectile projectile, ref NPC.HitModifiers modifiers)
    {
        ...
        ref AddableFloat scalingBonusDamage9 = ref modifiers.ScalingBonusDamage;

        if (Eclipsed > 0)
        {
            scalingBonusDamage9 += 0.1f;

            if (owner.active && !owner.dead && owner.HeldItem.ModItem is EclipsedDuo)
            {
                modifiers.FlatBonusDamage += owner.Additions().EclipsedDuoCounter;
            }
        }

        if (VoidEnergy > 0)
        {
            scalingBonusDamage9 += 0.35f;
        ...
        if (Wavebreaked > 0)
        {
            scalingBonusDamage9 += 0.15f;
            modifiers.FlatBonusDamage += 35;
        }

        if (StarKunai > 0)
        {
            scalingBonusDamage9 += 0.06f;
