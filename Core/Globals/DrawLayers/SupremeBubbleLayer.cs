...
public class SupremeBubbleLayer : PlayerDrawLayer
{
    ...
    protected override void Draw(ref PlayerDrawSet drawInfo)
    {
        ...
        Vector2 bubbleScale = Vector2.One *
                              (1f * 0.8f + MathF.Cos(Main.GlobalTimeWrappedHourly * 1.1f + drawInfo.drawPlayer.whoAmI) *
                                  0.04f);
        Vector2 scalingDirection = -Vector2.UnitY.RotatedBy(drawInfo.drawPlayer.whoAmI % 4 / 4f * MathHelper.TwoPi);
        bubbleScale += scalingDirection *
                       (float) Cos01(Main.GlobalTimeWrappedHourly * 3.1f + drawInfo.drawPlayer.whoAmI) * 0.16f;
        drawInfo.DrawDataCache.Add(new DrawData(texture, new Vector2(drawX, drawY), null, Color.White with { A = 0 },
            0f, texture.Size() * 0.5f, bubbleScale / 3, 0, 0f));
    }
}
