...
public sealed record ManagedILEdit(string Name, Action<ManagedILEdit> SubscriptionFunction, Action<ManagedILEdit> UnsubscriptionFunction, ManagedILManipulator EditingFunction)
{
    ...
    public void LogFailure(string reason) => ModContent.GetInstance<AdditionsMain>().Logger.Debug($"The IL edit of the name '{Name}' failed to load for the following reason:\n{reason}");

    private static void CacheEdit(ManagedILEdit edit) => GetEditListSafely(AdditionsMain.Instance.Name).Add(edit);
