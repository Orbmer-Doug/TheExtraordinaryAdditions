...
public class VerletSimulatedSegment(Vector2 _position, bool _locked = false, Vector2 _velocity = new())
{
    ...
    /// <summary>
    /// Accounts for tiles and liquids
    /// </summary>
    /// <param name="segments">The amount of segments wanted</param>
    /// <param name="segmentDistance">Distance apart from each other</param>
    /// <param name="loops">How many iterations to prevent one stick from being the wrong length</param>
    /// <param name="gravity">The amount of gravity to be added to a point</param>
    /// <returns></returns>
    public static List<VerletSimulatedSegment> AdvancedSimulation(List<VerletSimulatedSegment> segments, float segmentDistance, int loops = 10, float gravity = 0.3f)
    {
        ...
        foreach (VerletSimulatedSegment segment in segments)
        {
            ...
            {
                Vector2 positionBeforeUpdate = segment.position;
                segment.velocity = segment.position - segment.oldPosition;
        ...
        foreach (VerletSimulatedSegment segment in segments)
        {
            if (!segment.locked)
            {
                ...
                velocity = Collision.TileCollision(segment.position, velocity, SegmentWidth, SegmentHeight, true);

                Vector4 slope = Collision.SlopeCollision(segment.position, velocity, SegmentWidth, SegmentHeight, 1f);
                segment.position.X = slope.X;
                segment.position.Y = slope.Y;
                velocity.X = slope.Z;
                velocity.Y = slope.W;

                velocity = Collision.WaterCollision(segment.position, velocity, SegmentWidth, SegmentHeight, true, false, true);

                segment.position = segment.oldPosition + velocity;
            }
        }
