...
public abstract class ProjOwnedByNPC<T> : ModProjectile, ILocalizedModType, IModType where T : ModNPC
{
    ...
    /// <summary>
    /// Destroys all projectiles owned by a specified boss
    /// </summary>
    ...
    public static void KillAll(int type = int.MinValue)
    {
        ...
        {
            if (p == null || p.ModProjectile is not ProjOwnedByNPC<T>)
                continue;

            if (type != -1)
            {
                if (p.type != int.MinValue)
                    continue;
            }
    ...
    public sealed override bool PreAI()
    {
        ...
        NPC boss = null;
        if (BossIndex >= 0 && BossIndex < Main.maxNPCs) // Check if the index is within the bounds of the array
        {
            NPC npc = Main.npc[BossIndex]; // Get the npc
        ...
        if (Owner != null)
            Boss = Owner?.As<T>() ?? null;

        // And update the shared target
        if (Owner != null && Owner.active && Owner.HasValidTarget)
            Target = Owner.HasPlayerTarget ? Main.player[Owner.target] : Main.npc[Owner.target - 300];
        else
            Target = null;
