...
public sealed class Trail : IDisposable
{
    ...
    /// <summary>
    /// Initializes a new <see cref="Trail"/>.
    /// </summary>
    public Trail(VertexWidthFunction widthFunction, VertexColorFunction colorFunction,
    ...
    /// <param name="pixelated">For fixing zoom, because it is already applied for pixelation</param>
    public void DrawTrail(ManagedShader effect, ReadOnlySpan<Vector2> originalPositions, int totalTrailPoints = -1,
        bool smooth = false, bool pixelated = true)
    {
        if (Main.dedServ)
            return;
        ...
        // Apply effect
        effect.Render(transformMatrix: GetOrthographicMeshMatrix(!pixelated));
    ...
    /// <param name="effect">The shader to apply</param>
    /// <param name="originalPositions">The points</param>
    /// <param name="matrix">A custom matrix</param>
    /// <param name="totalTrailPoints">Increase for smoothness</param>
    /// <param name="smooth">Set to true for greater smoothness</param>
    public void DrawTrail(ManagedShader effect, ReadOnlySpan<Vector2> originalPositions, Matrix matrix,
        int totalTrailPoints = -1,
        bool smooth = false)
    {
        if (Main.dedServ)
            return;
