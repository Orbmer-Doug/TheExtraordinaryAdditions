...
public class ClothSimulation : IDisposable
{
    ...
    public void Draw(Texture2D texture)
    {
        void draw()
        {
            ...
            ManagedShader clothShader = AssetRegistry.GennedShaders.ClothShader;
            clothShader.TrySetParameter("size", new Vector2(_gridWidth, _gridHeight));
            clothShader.TrySetParameter("transformMatrix", GetOrthographicMeshMatrix());
...
foreach (Stick stick in _sticks)
{
    ...
    if (length > 0)
    {
        ...
        Main.spriteBatch.Draw(AssetRegistry.GennedTextures.Pixel, start, null, color, angle, SystemVector2.Zero,
...
public class qwerFlag : ModProjectile
{
    ...
    {
        sim?.Draw(AssetRegistry.GennedTextures.Background_CloudedCrater);
        return false;
    }
}
*/
