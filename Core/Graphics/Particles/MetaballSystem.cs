...
using static TheExtraordinaryAdditions.Core.Graphics.Particles.MetaballSystem;
...
public readonly struct InitMetaball(Vector2 vel, float scale, float opac, Color col, Vector2 size, float rot)
{
    ...
    public readonly Vector2 InitSize = size;
...
public class MetaballSystem : ModSystem
{
    ...
    public static BitmaskUtils.IndicesEnumerable ActiveMetaballs =>
        new BitmaskUtils.IndicesEnumerable(_presenceMask.AsSpan(0, _presenceMask.Length), MaxMetaballs);
    ...
    public override void OnModLoad()
    {
        ...
        Main.QueueMainThreadAction(() =>
        {
            ...
            _genericShader = AssetRegistry.GennedShaders.AMetaball;
    ...
    private void DrawMetaballsAtLayer(MetaballDrawLayers layer, bool resetSB = false)
    {
        ...
        foreach (MetaballTypes type in Enum.GetValues(typeof(MetaballTypes)))
        {
            ...
            ManagedShader shader = def.Shader == null
                ? _genericShader
                : AssetRegistry.GennedShaders.Shaders[def.Shader.Name];
...
public static class MetaballRegistry
{
    ...
    private static void PlasmaMetaballDefinition()
    {
        TypeDefinitions[(byte) MetaballTypes.Plasma] = new MetaballTypeDefinition(
            ...
            Draw: (ref m, _) => DefaultDraw(ref m, AssetRegistry.GennedTextures.NebulaGas1),
            DrawLayer: MetaballDrawLayers.AfterProjectiles,
            PrepareShader: static (shader, _) =>
            {
    ...
    private static void LavaMetaballDefinition()
    {
        TypeDefinitions[(byte) MetaballTypes.Lava] = new MetaballTypeDefinition(
            Shader: AssetRegistry.GennedShaders.LavaMetaball,
            Update: static (ref m) =>
            {
            ...
            DrawLayer: MetaballDrawLayers.AfterProjectiles,
            Draw: (ref m, _) => DefaultDraw(ref m, AssetRegistry.GennedTextures.BrightLight),
            PrepareShader: (shader, _) =>
            {
                shader.TrySetParameter("time", Main.GlobalTimeWrappedHourly);
                shader.TrySetParameter("threshold", .5f);
                shader.TrySetParameter("epsilon", .1f);
                shader.SetTexture(AssetRegistry.GennedTextures.BlobbyNoise, 2, SamplerState.LinearWrap);
                shader.Render();
            },
            OnCollision: (ref m) =>
            {
    ...
    private static void OnyxMetaballDefinition()
    {
        TypeDefinitions[(byte) MetaballTypes.Onyx] = new MetaballTypeDefinition(
            Shader: AssetRegistry.GennedShaders.OnyxMetaball,
            Update: static (ref m) =>
            {
                float timeRatio = (float) m.Time / m.Lifetime;
                m.Opacity = 1f - timeRatio;
            ...
            Draw: (ref m, _) => DefaultDraw(ref m, AssetRegistry.GennedTextures.BasicTriangle),
            DrawLayer: MetaballDrawLayers.AfterProjectiles,
            PrepareShader: static (shader, _) =>
            {
    ...
    public static void SpawnOnyxMetaball(Vector2 position, Vector2 velocity, int lifetime, int size, float rot = 0f)
    {
        Metaball metaball = new()
        {
            Position = position,
    ...
    private static void AbyssalMetaballDefinition()
    {
        TypeDefinitions[(byte) MetaballTypes.Abyssal] = new MetaballTypeDefinition(
            Shader: AssetRegistry.GennedShaders.AbyssalMetaball,
            Update: static (ref m) =>
            {
                float timeRatio = (float) m.Time / m.Lifetime;
                m.Opacity = 1f - timeRatio;
            ...
            Draw: (ref m, _) => DefaultDraw(ref m, AssetRegistry.GennedTextures.DetailedBlast),
            DrawLayer: MetaballDrawLayers.AfterProjectiles,
            PrepareShader: static (shader, _) =>
            {
                shader.TrySetParameter("globalTime", Main.GlobalTimeWrappedHourly);
                ...
                shader.SetTexture(AssetRegistry.GennedTextures.CausticNoise, 2, SamplerState.LinearWrap);
                shader.Render();
            },
            OnCollision: static (ref m) => { m.Velocity = Vector2.Zero; }
        );
    ...
    private static void GenediesMetaballDefinition()
    {
        TypeDefinitions[(byte) MetaballTypes.Genedies] = new MetaballTypeDefinition(
            Shader: AssetRegistry.GennedShaders.GenediesMetaball,
            Update: static (ref m) =>
            {
                float timeRatio = (float) m.Time / m.Lifetime;
                m.Opacity = 1f - timeRatio;
            ...
            {
                Texture2D texture = AssetRegistry.GennedTextures.NebulaGas2;
