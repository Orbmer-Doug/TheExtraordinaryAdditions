...
using static TheExtraordinaryAdditions.Core.Graphics.Particles.ShaderParticleSystem;
...
public class ShaderParticleSystem : ModSystem
{
    ...
    public static BitmaskUtils.IndicesEnumerable ActiveShaderParticles =>
        new BitmaskUtils.IndicesEnumerable(presenceMask.AsSpan(0, presenceMask.Length), MaxShaderParticles);
...
public static class ShaderParticleRegistry
{
    ...
    private static void DefaultDraw(ref ShaderParticle s, Texture2D tex)
    {
        Vector2 origin = tex.Size() * 0.5f;
        Vector2 scale = Vector2.One * (s.Size / tex.Size());
        Main.spriteBatch.PixelDraw(tex, s.Position, null, s.Color, s.Rotation, origin, scale);
    }
    
    private static void DefaultPrepare(ref ShaderParticleTypeDefinition def, Texture2D layer)
    {
        ManagedShader shader = def.Shader;
        Vector2 screenSize = Main.ScreenSize.ToVector2() / 2f;
        Vector2 layerScrollOffset = Main.screenPosition / screenSize + def.LayerOffset();
        shader.TrySetParameter("layerSize", layer.Size());
    ...
    private static void StygainParticleDefinition()
    {
        TypeDefinitions[(byte) ShaderParticleTypes.Stygain] = new ShaderParticleTypeDefinition(
            Shader: AssetRegistry.GennedShaders.EdgeDetectionShader,
            Update: static (ref s) =>
            {
                s.Color = MulticolorLerp(InverseLerp(0f, s.Size.Length(), 30f), Color.Black.Lerp(Color.DarkRed, .5f),
                    Color.DarkRed, Color.Red, Color.Crimson, Color.Crimson * 2f);
                s.Velocity *= .98f;
            ...
            Draw: static (ref s, _) =>
            {
                Texture2D tex = AssetRegistry.GennedTextures.BasicCircularCircle;
                ...
                Main.spriteBatch.PixelDraw(tex, s.Position, null, s.Color, s.Rotation, origin, scale / tex.Size());
            },
            DrawLayer: ShaderParticleDrawLayers.BeforeProjectiles,
            PrepareSB: null,
            PrepareShader: (_, _, type) => DefaultPrepare(ref TypeDefinitions[(byte)type], AssetRegistry.GennedTextures.TurbulentNoise),
            LayerOffset: static () =>
    ...
    private static void MoltenParticleDefinition()
    {
        TypeDefinitions[(byte) ShaderParticleTypes.Molten] = new ShaderParticleTypeDefinition(
            Shader: AssetRegistry.GennedShaders.AdditiveFusableParticleEdgeShader,
            Update: static (ref s) =>
            {
                ShaderParticleTypeDefinition def = TypeDefinitions[(byte) s.Type];
            ...
            ShouldKill: static (ref s) => s.Size.Length() <= 1,
            Draw: (ref s, _) => DefaultDraw(ref s, AssetRegistry.GennedTextures.BrightLight),
            ...
            PrepareShader: static (shader, _, _) =>
            {
    ...
    private static void EpidemicParticleDefinition()
    {
        TypeDefinitions[(byte) ShaderParticleTypes.Epidemic] = new ShaderParticleTypeDefinition(
            ...
            ShouldKill: static (ref s) => s.Size.Length() <= .2f,
            Draw: (ref s, _) => DefaultDraw(ref s, AssetRegistry.GennedTextures.BasicCircle),
            DrawLayer: ShaderParticleDrawLayers.BeforeProjectiles,
            PrepareSB: null,
            PrepareShader: (_, _, type) => DefaultPrepare(ref TypeDefinitions[(byte)type], AssetRegistry.GennedTextures.WavyBlotchNoise),
            LayerOffset: static () =>
                (Vector2.One * (float) Math.Cos(Main.GlobalTimeWrappedHourly * 0.041f) * 2f).RotatedBy(
                    (float) Math.Cos(Main.GlobalTimeWrappedHourly * 0.08f) * 0.97f),
    ...
    private static void CosmicParticleDefinition()
    {
        TypeDefinitions[(byte) ShaderParticleTypes.Cosmic] = new ShaderParticleTypeDefinition(
            ...
            Update: static (ref s) =>
            {
                s.Size *= .97f;
                s.Velocity *= .95f;
                s.Rotation = s.Velocity.ToRotation() + MathHelper.PiOver2;
                s.Color.A = 32;
            },
            ShouldKill: static (ref s) => s.Size.Length() <= 0.5f,
            Draw: (ref s, _) => DefaultDraw(ref s, AssetRegistry.GennedTextures.BasicCircle),
            DrawLayer: ShaderParticleDrawLayers.BeforeProjectiles,
            PrepareSB: null,
            PrepareShader: (_, _, type) => DefaultPrepare(ref TypeDefinitions[(byte)type], AssetRegistry.GennedTextures.PurpleNebulaBright),
            LayerOffset: static () => Vector2.UnitX * Main.GlobalTimeWrappedHourly * 0.03f,
            EdgeColor: Color.BlueViolet
        );
    }

    public static void SpawnCosmicParticle(Vector2 position, Vector2 velocity, Vector2 size)
    {
        ShaderParticle particle = new()
        {
            Position = position,
