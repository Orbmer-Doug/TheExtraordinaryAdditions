...
public class ShaderParticleSystem : ModSystem
{
    ...
    public static BitmaskUtils.BitmaskEnumerable ActiveShaderParticles =>
        new BitmaskUtils.BitmaskEnumerable(presenceMask.AsSpan(0, presenceMask.Length), MaxShaderParticles);
        ...
        foreach (ShaderParticleTypes type in Enum.GetValues(typeof(ShaderParticleTypes)))
        {
            ...
            foreach (int index in ActiveShaderParticles)
            {
                ...
                else
                {
                    ...
                    Main.spriteBatch.PixelDraw(def.Texture, particle.Position, null, particle.Color, 0f, origin, scale);
                }
            }
    ...
    private static void StygainParticleDefinition()
    {
        TypeDefinitions[(byte) ShaderParticleTypes.Stygain] = new ShaderParticleTypeDefinition(
            ...
            Draw: static (ref s, _) =>
            {
                ...
                Main.spriteBatch.PixelDraw(def.Texture, s.Position, null, s.Color, s.Rotation, origin,
                    scale / def.Texture.Size());
            },
