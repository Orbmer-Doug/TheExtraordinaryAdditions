...
namespace TheExtraordinaryAdditions.Common.Particles.Particle;
...
public sealed class ParticleSystem : ModSystem
{
    ...
    public static BitmaskUtils.BitmaskEnumerable ActiveParticles =>
        new BitmaskUtils.BitmaskEnumerable(presenceMask.AsSpan(0, presenceMask.Length), MaxParticles);
        ...
        foreach (BlendState blendState in SupportedBlendStates)
        {
            ...
            foreach (int index in ActiveParticles)
            {
                ...
                if (type != DrawTypes.Manual)
                {
                    ...
                    switch (type)
                    {
                        case DrawTypes.Pixelize when def.IsPrimitive:
...
public static class ParticleRegistry
{
    ...
    private static void BloomLineParticleDefinition()
    {
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                {
                    sb.DrawBetter(texture, p.Position, null, p.Color, p.Rotation, texture.Size() / 2, scale * i);
                    sb.DrawBetter(texture, p.Position, null, p.Color, p.Rotation, texture.Size() / 2,
            ...
            draw: static (ref p, sb) =>
            {
                ...
                if (custom.TrailLength > 0)
                {
                    ...
                    for (int i = 0; i < custom.TrailLength && i < oldPos.Length; i++)
                    {
                        ...
                        if (custom.Intense)
                            sb.DrawBetter(bloom, p.Position, null, p.Color * p.Opacity * 0.55f, p.Rotation, bloomOrigin,
                                custom.BloomScale * 0.15f);
                        sb.DrawBetter(pixel, oldPos[i], null, p.Color * p.Opacity, p.Rotation, pixelOrigin,
                            p.Scale * 6f * completion, direction);
                    }
                }
                else
                {
                    sb.DrawBetter(bloom, p.Position, null, p.Color * p.Opacity * (custom.Intense ? 1.1f : 0.55f),
                        p.Rotation, bloomOrigin, custom.BloomScale * (custom.Intense ? .25f : .15f));
                    sb.DrawBetter(pixel, p.Position, null, p.Color * p.Opacity, p.Rotation, pixelOrigin, p.Scale * 6f,
                        direction);
                }
            },
            drawType: DrawTypes.Pixelize,
        ...
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                if (custom.AuraCol.HasValue)
                {
                    ...
                    sb.DrawBetterRect(aura, target, null, custom.AuraCol.Value * p.Opacity, p.Rotation, orig);
                    sb.DrawBetterRect(aura, target, null, custom.AuraCol.Value * p.Opacity, p.Rotation, orig);
                }

                sb.DrawBetterRect(tex, target, null, p.Color * p.Opacity, p.Rotation, tex.Size() / 2f);
            },
            drawType: DrawTypes.Pixelize,
            canCull: false
        ));
    ...
    private static void SparkParticleDefinition()
    {
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                sb.DrawBetter(texture, p.Position, null, Color.Lerp(Color.White, p.Color, 0.4f) * p.Opacity, p.Rotation,
    ...
    public static void SpawnSparkleParticle(Vector2 position, Vector2 velocity, int lifetime, float scale, Color color,
        Color bloomColor, float bloomScale = 1f, float spin = 0.1f)
    {
        ParticleData particle = new()
        {
            Position = position,
    ...
    private static void SquishyLightParticleDefinition()
    {
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                sb.DrawBetter(texture, p.Position, null, p.Color * p.Opacity * 0.8f, rotation, texture.Size() / 2,
                    scale * 1.1f);
                sb.DrawBetter(texture, p.Position, null, Color.White * p.Opacity * 0.9f, rotation, texture.Size() / 2,
                    scale);
            },
            drawType: DrawTypes.Pixelize
        ));
            ...
            draw: static (ref p, sb) =>
            {
                ...
                if (custom.TrailLength > 0)
                {
                    ...
                    for (int i = 0; i < custom.TrailLength && i < oldPos.Length; i++)
                    {
                        ...
                        float completion = 1f - InverseLerp(0f, custom.TrailLength, i);
                        sb.DrawBetter(bloom, old, null, custom.BloomColor * p.Opacity * completion, p.Rotation,
                            bloom.Size() / 2, scale * 0.14f * completion);
                        sb.DrawBetter(texture, old, null, p.Color * p.Opacity, p.Rotation, orig,
                            scale * 7 * completion);
                    }
                }
                else
                {
                    sb.DrawBetter(bloom, p.Position, null, custom.BloomColor * p.Opacity, p.Rotation, bloom.Size() / 2,
                        scale * 0.14f);
                    sb.DrawBetter(texture, p.Position, null, p.Color * p.Opacity, p.Rotation, orig, scale * 7);
                }
            },
            drawType: DrawTypes.Pixelize
        ));
        ...
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                for (int i = -1; i <= 1; i++)
                {
                    ...
                    Vector2 offset = Utils.RotatedBy(PolarVector(1f, p.Rotation), MathHelper.PiOver2) * i;
                    offset *= custom.Strength;
                    sb.DrawBetter(texture, p.Position + offset, custom.TechFrame,
                        p.Color.MultiplyRGB(aberrationColor) * p.Opacity, p.Rotation, custom.TechFrame.Size() / 2f,
                        p.Scale);
                }

                sb.DrawBetter(texture, p.Position, custom.TechFrame, p.Color * p.Opacity, p.Rotation,
        ...
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                SpriteEffects flip = Main.GlobalTimeWrappedHourly % 30f < 15f
                    ...
                    : SpriteEffects.None;
                sb.DrawBetter(texture, p.Position + shake, null, p.Color * p.Opacity * 0.6f, p.Rotation, origin,
                    custom.Squish * p.Scale, flip);
                sb.DrawBetter(texture, p.Position, null, drawColor * p.Opacity, p.Rotation, origin,
                    custom.Squish * p.Scale, flip);
            },
            drawType: DrawTypes.Pixelize
        ));
    ...
    private static void TwinkleParticleDefinition()
    {
        RegisterDefinition(new ParticleDefinition(
            ...
            draw: static (ref p, sb) =>
            {
                ...
                sb.DrawBetter(AssetRegistry.GetTexture(AdditionsTexture.GlowSoft), p.Position, null,
                    backglowBloomColor * 0.83f, 0f, AssetRegistry.GetTexture(AdditionsTexture.GlowSoft).Size() * 0.5f,
                    scale * 7.2f);
                sb.DrawBetter(AssetRegistry.GetTexture(AdditionsTexture.BloomFlare), p.Position, null, color,
                    p.Rotation - Main.GlobalTimeWrappedHourly * 0.9f,
                    AssetRegistry.GetTexture(AdditionsTexture.BloomFlare).Size() * 0.5f, scale * 0.42f);
                sb.DrawBetter(AssetRegistry.GetTexture(AdditionsTexture.BloomFlare), p.Position, null, color,
                ...
                for (int i = 0; i < instanceCount; i++)
                {
                    ...
                    for (float s = 1f; s > 0.3f; s -= 0.2f)
                        sb.DrawBetter(texture, p.Position, null, color, rotationOffset, texture.Size() * 0.5f,
