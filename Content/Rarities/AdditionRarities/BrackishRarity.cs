...
public class BrackishRarity : ModRarity
{
    ...
    public static void DrawCustomTooltipLine(DrawableTooltipLine tooltipLine)
    {
        ...
        if (Main.rand.NextBool(6))
            CustomRaritySystem.Spawn(ref WatorList, new RarityDroplet(rect.RandomRectangle(),
                Vector2.UnitY * Main.rand.NextFloat(.1f, 1.4f),
                Main.rand.Next(90, 150), Main.rand.NextFloat(.4f, .9f), AbyssalCurrents.BrackishPalette[2]));

        // Draw the base tooltip text and glow.
        CustomRaritySystem.DrawTextWithGlow(tooltipLine, AbyssalCurrents.BrackishPalette[0],
            AbyssalCurrents.BrackishPalette[1], Color.Black);
