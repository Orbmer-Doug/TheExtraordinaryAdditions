...
public static class ToolModifierUtils
{
    public static Point GetTileTarget(Player player)
    {
        return Main.SmartCursorWanted
            ? new Point(Main.SmartCursorX, Main.SmartCursorY)
            : new Point(Player.tileTargetX, Player.tileTargetY);
    }

    internal static MethodInfo MineMethod => typeof(Player).GetMethod("ItemCheck_UseMiningTools_ActuallyUseMiningTool",
        BindingFlags.NonPublic | BindingFlags.Instance);

    internal static MethodInfo WallMethod => typeof(Player).GetMethod("ItemCheck_UseMiningTools_TryFindingWallToHammer",
        BindingFlags.Static | BindingFlags.NonPublic | BindingFlags.Instance);

    public static void Mine(Player player, Item tool, bool hammer = false, bool right = false,
        Point? overrideTileTarget = null)
    {
            ...
            if (canHitWalls == true)
            {
                ...
                void FindWall(int wx, int wy)
                {
                    ...
                    if (tile.WallType > 0 && (!tile.Active() ||
                                              wallX != tileTarget.X || wallY != tileTarget.Y ||
                                              (!Main.tileHammer[tile.TileType] && !player.poundRelease))
