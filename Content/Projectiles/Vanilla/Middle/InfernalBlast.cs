...
public class InfernalBlast : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            Vector2 dustSpawnOffset = Main.rand.NextVector2Circular(1f, 1f) *
            ...
            dustVelocity = dustVelocity *
                           MathHelper.Lerp(3f, 6f, Utils.GetLerpValue(0.05f, 0.85f, val2.Length(), false));

            Vector2 pos = Projectile.Center + dustSpawnOffset;
            Color color = MulticolorLerp(Main.rand.NextFloat(0.2f, 0.8f), Color.Red, Color.OrangeRed, Color.IndianRed,
                Color.DarkRed, Color.Orange, Color.DarkOrange, Color.OrangeRed * 1.6f);

            ParticleRegistry.SpawnMistParticle(pos, dustVelocity, Main.rand.NextFloat(.7f, 1.5f), color,
                Color.Transparent, 190, Main.rand.NextFloat(-.2f, .2f));
