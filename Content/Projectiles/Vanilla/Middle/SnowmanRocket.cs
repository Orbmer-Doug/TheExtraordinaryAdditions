...
public class SnowmanRocket : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Time > 30f)
        {
            if (NPCTargeting.TryGetClosestNPC(new(Projectile.Center, 500, true), out NPC target))
                Projectile.velocity = Vector2.Lerp(Projectile.velocity,
                    Projectile.SafeDirectionTo(target.Center + target.velocity) * 16f, 1f / 12f);
        }

        if (Time % 4 == 3)
        {
            ParticleRegistry.SpawnBloomPixelParticle(Back,
                -Projectile.velocity.RotatedByRandom(.5f) * Main.rand.NextFloat(.35f, .7f), Main.rand.Next(20, 30),
        ...
        if (Projectile.velocity.X < 0f)
        {
            ...
            Projectile.rotation =
                (float) Math.Atan2(-Projectile.velocity.Y, -Projectile.velocity.X) - MathHelper.PiOver2;
        }
        else
        {
            Projectile.spriteDirection = 1;
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        switch (State)
        {
            ...
            case (int) RocketType.Dry:
                ...
                if (Main.netMode != NetmodeID.MultiplayerClient)
                    Projectile.Kill_DirtAndFluidProjectiles_RunDelegateMethodPushUpForHalfBricks(posit, 3.5f,
                        DelegateMethods.SpreadDry);
            ...
            case (int) RocketType.Wet:
                ...
                Projectile.Resize(SmallSize, SmallSize);
                if (Main.netMode != NetmodeID.MultiplayerClient)
                    Projectile.Kill_DirtAndFluidProjectiles_RunDelegateMethodPushUpForHalfBricks(posit, 3f,
                        DelegateMethods.SpreadWater);
        ...
        void Kaboom()
        {
            ...
            bool wallSplode2 =
                Projectile.ShouldWallExplode(pos, (int) boomSize, leftTileX, rightTileX, downTileY, upTileY);
            Projectile.ExplodeTiles(pos, (int) boomSize, leftTileX, rightTileX, downTileY, upTileY, wallSplode2);
        }

        for (int i = 0; i < 40; i++)
        {
            Vector2 vel = (MathHelper.TwoPi * (i / 40) + RandomRotation()).ToRotationVector2() *
                          Main.rand.NextFloat(1f, 5f);
            int life = Main.rand.Next(20, 40);
            float scale = Main.rand.NextFloat(.3f, .5f);
            Color color = MulticolorLerp(Main.rand.NextFloat(), Color.LightCyan, Color.Cyan, Color.SkyBlue,
                Color.DeepSkyBlue, Color.BlueViolet);
            switch (Projectile.height)
            {
                case SmallSize:
                    if (i == 0)
                    {
                        ...
                        ParticleRegistry.SpawnPulseRingParticle(pos, Vector2.Zero, life / 2, 0f, Vector2.One, 0f,
                            SmallSize, color);
                    }

                    ParticleRegistry.SpawnGlowParticle(pos, vel, life, scale * 70f, color, Main.rand.NextFloat(.4f, 1f),
                        true);
                    ParticleRegistry.SpawnBloomLineParticle(pos, vel * 2, life / 2, scale, color);
                    ParticleRegistry.SpawnCloudParticle(pos, vel, color, Color.DarkBlue, life, scale * 120f,
                        Main.rand.NextFloat(.5f, .8f), 2);
                    break;
                case RegularSize:
                    if (i == 0)
                    {
                    ...
                    ParticleRegistry.SpawnGlowParticle(pos, vel * 1.2f, life + 3, scale * 70f * 1.4f, color,
                        Main.rand.NextFloat(.4f, 1f), true);
                    ParticleRegistry.SpawnBloomLineParticle(pos, vel * 2.2f, life / 2, scale * 1.2f, color);
                    ParticleRegistry.SpawnCloudParticle(pos, vel * 1.1f, color, Color.DarkBlue, life, scale * 120f,
                        Main.rand.NextFloat(.5f, .8f), 2);
                    break;
                case BigSize:
                    if (i == 0)
                    ...
                    ParticleRegistry.SpawnGlowParticle(pos, vel, life + 5, scale * 70f * 1.85f, color,
                        Main.rand.NextFloat(.4f, 1f), true);
                    ParticleRegistry.SpawnBloomLineParticle(pos, vel * 2.5f, life / 2, scale * 1.4f, color);
                    ParticleRegistry.SpawnCloudParticle(pos, vel * 1.4f, color, Color.DarkBlue, life, scale * 120f,
                        Main.rand.NextFloat(.6f, .8f), 2);
                    break;
                case HugeSize:
                    if (i == 0)
                    {
                    ...
                    ParticleRegistry.SpawnGlowParticle(pos, vel * 1.4f, life + 20, scale * 70f * 2f, color,
                        Main.rand.NextFloat(.4f, 1f), true);
                    ParticleRegistry.SpawnBloomLineParticle(pos, vel * 3f, life / 2, scale * 1.6f, color);
                    ParticleRegistry.SpawnCloudParticle(pos, vel * 1.8f, color, Color.DarkBlue, life, scale * 120f,
                        Main.rand.NextFloat(.7f, .9f), 2);
                    break;
            }
        }
    ...
    private Color ColorFunct(SystemVector2 c, Vector2 position)
    {
        ...
        Color dark = Color.DarkBlue.Lerp(Color.DarkViolet.Lerp(Color.DarkBlue, .8f),
            Cos01(Main.GlobalTimeWrappedHourly));
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Main.spriteBatch.Draw(Tex, pos, null, lightColor * Projectile.Opacity, Projectile.rotation, orig,
            Projectile.scale,
