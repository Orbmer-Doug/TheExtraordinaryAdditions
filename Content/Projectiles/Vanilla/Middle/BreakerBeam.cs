...
public class BreakerBeam : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        Vector2 middle = (start + end) * 0.5f +
                         (Direction - MathHelper.PiOver2 + (.18f * Projectile.direction)).ToRotationVector2() *
        ...
        if (Time % 2 == 1)
        {
            ParticleRegistry.SpawnSparkParticle(slashPoints.Points[Main.rand.Next(slashPoints.Count)],
                -Projectile.velocity * Main.rand.NextFloat(.5f, .7f),
                Main.rand.Next(20, 40), Main.rand.NextFloat(.4f, .6f),
                Color.Lerp(Color.White, Main.rand.NextFloat(0f, .3f)));
        }

        if (Time % 4 == 3)
        {
            ParticleRegistry.SpawnGlowParticle(slashPoints.Points[Main.rand.Next(slashPoints.Count)],
                -Projectile.velocity.RotatedByRandom(.1f) * Main.rand.NextFloat(.2f, .4f),
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (int i = 0; i < 30; i++)
        {
            ParticleRegistry.SpawnSquishyLightParticle(slashPoints.Points[Main.rand.Next(slashPoints.Count)],
                Projectile.velocity * Main.rand.NextFloat(.3f, .8f), Main.rand.Next(20, 34),
                Main.rand.NextFloat(.6f, 1f), Color, 1f, 2f, 5f);
        }

        ParticleRegistry.SpawnTwinkleParticle(Projectile.Center + norm * 20f, Vector2.Zero, 20,
    ...
    public float WidthFunct(float c) =>
        Convert01To010(c) * Projectile.scale * Projectile.Opacity * Projectile.width / 2;
