...
public class HeatRayHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        Projectile.Center = Owner.GetFrontHandPositionImproved() + PolarVector(15f, Projectile.rotation) +
                            PolarVector(5f * Owner.direction * Owner.gravDir, Projectile.rotation - MathHelper.PiOver2);
            ...
            LaserResource.CanFire(Owner) && TryUseMana(false))
        {
            ...
            for (int i = 0; i < 8; i++)
                ParticleRegistry.SpawnSparkParticle(pos, vel.RotatedByRandom(.4f) * Main.rand.NextFloat(.1f, .6f),
                    Main.rand.Next(30, 40), Main.rand.NextFloat(.4f, .5f), Color.Yellow);
        }
        ...
        if (this.RunLocal() && Modded.SafeMouseRight.Current && Time % wait == wait - 1 && LaserResource.CanFire(Owner))
        {
            if (TryUseMana())
            {
                ...
                for (int i = 0; i < 15; i++)
                    ParticleRegistry.SpawnSparkParticle(pos, vel.RotatedByRandom(.4f) * Main.rand.NextFloat(.1f, .6f),
                        Main.rand.Next(30, 40), Main.rand.NextFloat(.5f, .6f), Color.OrangeRed);
            }
        }
