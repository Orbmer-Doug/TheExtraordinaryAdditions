...
public class AmethystStaffHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        Vector2 tip = Projectile.RotHitbox().TopRight +
                      PolarVector(Item.height, Projectile.rotation - MathHelper.PiOver4);
        ...
        if (Modded.SafeMouseLeft.Current && ShootDelay <= 0 && this.RunLocal())
        {
            ...
            for (int i = 0; i < 12; i++)
                ParticleRegistry.SpawnSparkParticle(tip, vel.RotatedByRandom(.5f) * Main.rand.NextFloat(.3f, .6f),
                    Main.rand.Next(14, 25), Main.rand.NextFloat(.4f, .5f), Color.Purple);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Main.spriteBatch.DrawBetter(tex, Projectile.Center, frame, Projectile.GetAlpha(lightColor),
            Projectile.rotation + off, orig, 1, fx);
        return false;
    }
}
...
public class AmethystBoltOverride : ProjectileOverride
{
    ...
    {
        ParticleRegistry.SpawnGlowParticle(proj.Center, -proj.velocity * Main.rand.NextFloat(.1f, .3f),
            Main.rand.Next(20, 30), proj.width * Main.rand.NextFloat(.8f, 1.4f), Color.Purple, 1.7f);
        ParticleRegistry.SpawnGlowParticle(proj.Center, proj.velocity * Main.rand.NextFloat(.1f, .3f),
            Main.rand.Next(20, 30), proj.width * Main.rand.NextFloat(.8f, 1.4f), Color.Magenta, 1.7f);
    ...
    public override void OnKill(Projectile proj)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            Vector2 vel = (MathHelper.TwoPi * i / 20 + RandomRotation()).ToRotationVector2() *
...
public class TopazStaffHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        Vector2 pos = Projectile.RotHitbox().TopRight +
                      PolarVector(Item.width - 10, Projectile.rotation - MathHelper.PiOver4);
        Owner.SetFrontHandBetter(0, Projectile.rotation - MathHelper.PiOver4);
        Projectile.Center = Owner.GetFrontHandPositionImproved() + Vector2.UnitY * Owner.gfxOffY;
        ...
        if (Modded.MouseLeft.Current && Charge < MaxCharge && this.RunLocal())
        {
            if (Charge % 2 == 1)
                ParticleRegistry.SpawnBloomPixelParticle(pos + Main.rand.NextVector2CircularLimited(40f, 40f, .4f, 1f),
                    Main.rand.NextVector2Circular(2f, 2f), Main.rand.Next(14, 20),
                    Main.rand.NextFloat(.3f, .5f) * Completion, Color.Orange, Color.DarkOrange, pos, Completion, 2);
        ...
        if (Charge > 20f && Modded.CanUseMouseButton && Modded.SafeMouseLeft.JustReleased && this.RunLocal())
        {
            ...
            for (int i = 0; i < 12; i++)
                ParticleRegistry.SpawnGlowParticle(pos,
                    vel.RotatedByRandom(.5f) * Main.rand.NextFloat(.3f, .6f) * Completion,
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        if (Completion > 0f)
        {
            void draw()
            {
                ...
                Main.spriteBatch.DrawBetter(star, pos, null, Color.Orange * Completion, rot, star.Size() / 2,
                    .15f * Completion, 0);
                Main.spriteBatch.DrawBetter(star, pos, null, Color.Orange.Lerp(Color.White, .5f) * Completion, rot,
                    star.Size() / 2, .1f * Completion, 0);
            }
...
public class TopazBoltOverride : ProjectileOverride
{
    ...
    public override bool PreAI(Projectile proj)
    {
        ...
        ParticleRegistry.SpawnGlowParticle(proj.Center, proj.velocity * Main.rand.NextFloat(.2f, .5f),
            Main.rand.Next(20, 30), Main.rand.NextFloat(14.3f, 16.4f) * power, Color.Orange, 1.3f);
        ParticleRegistry.SpawnDustParticle(proj.Center,
            -proj.velocity.RotatedByRandom(.3f) * Main.rand.NextFloat(.3f, .4f), Main.rand.Next(30, 40),
            Main.rand.NextFloat(.2f, .4f) * power, Color.Orange, .1f, false, true);
    ...
    public override void OnKill(Projectile proj)
    {
        ...
        for (int i = 0; i < 20; i++)
        {
            ParticleRegistry.SpawnGlowParticle(proj.Center,
                proj.velocity.RotatedByRandom(.2f) * Main.rand.NextFloat(.2f, .5f),
...
public class SapphireStaffHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        if (Modded.SafeMouseLeft.Current && ShootDelay <= 0 && this.RunLocal())
        {
            ...
            for (int i = 0; i < 12; i++)
                ParticleRegistry.SpawnBloodParticle(pos, vel.RotatedByRandom(.4f) * Main.rand.NextFloat(.2f, .65f),
                    Main.rand.Next(18, 28), Main.rand.NextFloat(.4f, .5f), Color.Blue);
...
public class SapphireBoltOverride : ProjectileOverride
{
    ...
    public override bool PreAI(Projectile proj)
    {
        ParticleRegistry.SpawnCloudParticle(proj.Center, proj.velocity * Main.rand.NextFloat(-.1f, .1f),
            Color.LightBlue, Color.DarkBlue, Main.rand.Next(18, 24),
            Main.rand.NextFloat(proj.width - 2f, proj.width) * 3f, Main.rand.NextFloat(.5f, .7f), 1);
    ...
    public override void OnKill(Projectile proj)
    {
        for (int i = 0; i < 12; i++)
        {
            ParticleRegistry.SpawnGlowParticle(proj.Center, Main.rand.NextVector2Circular(3f, 3f),
                Main.rand.Next(20, 30), Main.rand.NextFloat(13f, 18f), Color.DarkBlue);
        }
    }
...
public class EmeraldStaffHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        if (Modded.SafeMouseLeft.Current && ShootDelay <= 0 && this.RunLocal())
        {
            ...
            Projectile.NewProj(pos, vel, Item.shoot, Item.damage, Item.knockBack, Owner.whoAmI);
            for (int i = 0; i < 12; i++)
                ParticleRegistry.SpawnBloomPixelParticle(pos, vel.RotatedByRandom(.5f) * Main.rand.NextFloat(.4f, .65f),
...
public class EmeraldBoltOverride : ProjectileOverride
{
    ...
    public override bool PreAI(Projectile proj)
    {
        for (int i = 0; i < 3; i++)
            ParticleRegistry.SpawnGlowParticle(proj.Center,
                -proj.velocity.RotatedByRandom(.2f) * Main.rand.NextFloat(.1f, .2f),
    ...
    public override void OnKill(Projectile proj)
    {
        for (int i = 0; i < 18; i++)
            ParticleRegistry.SpawnGlowParticle(proj.Center,
                proj.velocity.RotatedByRandom(.5f) * Main.rand.NextFloat(.01f, .1f), Main.rand.Next(40, 50),
                Main.rand.NextFloat(14f, 21f), Color.LawnGreen, 1.5f);
    }
...
public class AmberBoltOverride : ProjectileOverride
{
    ...
    {
        ParticleRegistry.SpawnGlowParticle(proj.Center,
            -proj.velocity.RotatedByRandom(.2f) * Main.rand.NextFloat(.2f, .4f), Main.rand.Next(20, 30),
            Main.rand.NextFloat(18f, 28f), col);
        ParticleRegistry.SpawnHeavySmokeParticle(proj.Center, -proj.velocity * Main.rand.NextFloat(.2f, .4f),
            Main.rand.Next(20, 30), Main.rand.NextFloat(.2f, .4f), col);
    ...
    public override void OnKill(Projectile proj)
    {
        for (int i = 0; i < 15; i++)
        {
            Vector2 vel = (MathHelper.TwoPi * i / 15 + RandomRotation()).ToRotationVector2() *
                          Main.rand.NextFloat(2f, 3f);
            ParticleRegistry.SpawnDustParticle(proj.Center, vel, Main.rand.Next(20, 30), Main.rand.NextFloat(.3f, .5f),
                col, .1f, false, true);
        }
    }
...
public class RubyStaffHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        if (Modded.SafeMouseLeft.Current && ShootDelay <= 0 && this.RunLocal())
        {
            ...
            Projectile.NewProj(pos, vel, Item.shoot, Item.damage, Item.knockBack, Owner.whoAmI);
            for (int i = 0; i < 12; i++)
                ParticleRegistry.SpawnHeavySmokeParticle(pos, vel.RotatedByRandom(.5f) * Main.rand.NextFloat(.3f, .6f),
                    Main.rand.Next(14, 25), Main.rand.NextFloat(.4f, .5f), Color.Red);
            SoundID.Item43.Play(pos);
...
public class RubyBoltOverride : ProjectileOverride
{
    ...
    public override bool PreAI(Projectile proj)
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            ParticleRegistry.SpawnHeavySmokeParticle(proj.Center, vel.RotatedByRandom(.15f), Main.rand.Next(20, 30),
                Main.rand.NextFloat(.1f, .2f), Color.Crimson);
            ParticleRegistry.SpawnGlowParticle(proj.Center, vel, Main.rand.Next(16, 23),
                Main.rand.NextFloat(20.3f, 36.4f), Color.Red, .7f);
        }
    ...
    public override void OnKill(Projectile proj)
    {
        for (int i = 0; i < 18; i++)
        {
            ParticleRegistry.SpawnHeavySmokeParticle(proj.Center,
                proj.velocity.RotatedByRandom(.35f) * Main.rand.NextFloat(.3f, .4f), Main.rand.Next(25, 38),
                Main.rand.NextFloat(.2f, .4f), Color.Red);
            ParticleRegistry.SpawnGlowParticle(proj.Center,
                proj.velocity.RotatedByRandom(.2f) * Main.rand.NextFloat(.3f, .5f), Main.rand.Next(20, 28),
                Main.rand.NextFloat(20f, 30f), Color.Red, .5f);
        }
    }
}
...
public class DiamondStaffHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        if (Modded.SafeMouseLeft.Current && ShootDelay <= 0 && this.RunLocal())
        {
            Vector2 vel = Projectile.velocity * Item.shootSpeed;
            Projectile p =
                Main.projectile[Projectile.NewProj(pos, vel, Item.shoot, Item.damage, Item.knockBack, Owner.whoAmI)];
            p.ai[1] = Item.shootSpeed;

            for (int i = 0; i < 12; i++)
                ParticleRegistry.SpawnSparkleParticle(pos, vel.RotatedByRandom(.5f) * Main.rand.NextFloat(.3f, .6f),
                    Main.rand.Next(18, 30), Main.rand.NextFloat(.4f, .5f), Color.White, Color.LightCyan, 1.2f);
...
public class DiamondBoltOverride : ProjectileOverride
{
    ...
    public override bool PreAI(Projectile proj)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            if (i == 0 && Main.rand.NextBool(3))
                ParticleRegistry.SpawnBloomPixelParticle(proj.Center,
                    -proj.velocity.RotatedByRandom(.24f) * Main.rand.NextFloat(.35f, .5f),
        ...
        if (time.BetweenNum(15f, 40f))
        {
            ...
            {
                proj.velocity = Vector2.SmoothStep(proj.velocity,
                    proj.SafeDirectionTo(owner.Additions().MouseWorld) * proj.ai[1], .1f);
    ...
    public override void OnKill(Projectile proj)
    {
        for (int i = 0; i < 16; i++)
            ParticleRegistry.SpawnGlowParticle(proj.Center,
                (MathHelper.TwoPi * i / 16).ToRotationVector2() * Main.rand.NextFloat(1f, 2f), Main.rand.Next(20, 30),
                Main.rand.NextFloat(22f, 30f), Color.WhiteSmoke);
    }
}
