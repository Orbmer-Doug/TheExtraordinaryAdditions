    ...
    public override void AI()
    {
        ...
        if (Projectile.ai[1] == 1f)
        {
            if (NPCTargeting.TryGetClosestNPC(new(Projectile.Center, 240, true), out NPC target))
                Projectile.velocity = Vector2.SmoothStep(Projectile.velocity,
                    Projectile.SafeDirectionTo(target.Center) * 10f, .09f);
        }
        ...
        for (int a = 0; a < 2; a++)
        {
            for (int b = 0; b < 3; b++)
            {
                ...
                int num135 = 6;
                int num136 = Dust.NewDust(new Vector2(Projectile.position.X + num135, Projectile.position.Y + num135),
                    Projectile.width - num135 * 2, Projectile.height - num135 * 2, DustID.DungeonWater, 0f, 0f, 100,
                    default, 1.2f);
            ...
            {
                int num137 = 6;
                int num138 = Dust.NewDust(new Vector2(Projectile.position.X + num137, Projectile.position.Y + num137),
                    Projectile.width - num137 * 2, Projectile.height - num137 * 2, DustID.DungeonWater, 0f, 0f, 100,
                    default, 0.75f);
    ...
    public override void OnKill(int timeLeft)
    {
        for (int a = 0; a < 30; a++)
        {
            Dust.NewDust(new Vector2(Projectile.position.X, Projectile.position.Y), Projectile.width, Projectile.height,
                DustID.WaterCandle, 0f, 0f, 150, default, 1.1f);
        }
