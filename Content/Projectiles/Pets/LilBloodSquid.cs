...
public class LilBloodSquid : ModProjectile, ILocalizedModType, IModType
{
    ...
    public override void AI()
    {
        ...
        Vector2 idealPosition = Owner.MountedCenter - Vector2.UnitY * MathHelper.Lerp(20f, 40f, Sin01(Time * .06f)) -
        ...
        if (Utils.NextBool(Main.rand, 50))
        {
            int d1 = Dust.NewDust(Projectile.Center, Projectile.width, Projectile.height, DustID.Blood, 0f, 0f, 100,
                default(Color), 1.5f);
            int d2 = Dust.NewDust(Projectile.Center, Projectile.width, Projectile.height, DustID.CrimsonPlants, 0f, 0f,
                170, default(Color), 0.5f);
