...
public class ConsumingVoid : ModProjectile, ILocalizedModType, IModType
{
    ...
    public override void AI()
    {
        ...
        if (target.CanHomeInto())
        {
            Projectile.velocity = Vector2.Lerp(Projectile.velocity,
                Projectile.SafeDirectionTo(target.Center) * (26f / Projectile.MaxUpdates), .02f);
        }
        else
        ...
        Projectile.scale = Utils.Remap(Time, 0f, Projectile.MaxUpdates * 15f, 0.01f, 1.5f, true) *
                           Utils.GetLerpValue(0f, Projectile.MaxUpdates * 16f, Projectile.timeLeft, true);
        Projectile.ExpandHitboxBy((int) (Projectile.scale * 62f));

        if (Time % 3f == 0f)
        {
            // Create suck energy particles.
            Vector2 energySpawnPosition = Projectile.Center +
                                          Main.rand.NextVector2Unit() * Projectile.width *
                                          Main.rand.NextFloat(0.97f, 2.1f);
            ...
            Particles.Add(new()
            {
                ...
                Opacity = 1f,
                DrawColor = Color.Lerp(Color.Lerp(Color.BlueViolet, Color.Violet, Main.rand.NextFloat()), Color.White,
    ...
    public Color ColorFunct(SystemVector2 c, Vector2 position)
    {
        ...
        return MulticolorLerp(MathHelper.Clamp(AperiodicSin(Time * .08f) * .5f, 0f, 1f), Color.Violet, Color.BlueViolet,
                   Color.MediumPurple, Color.Purple, Color.DarkViolet) * Projectile.Opacity *
               MathHelper.SmoothStep(1f, 0f, c.X) * Utils.GetLerpValue(0.04f, 0.2f, c.X, true) * velFade;
    }
    ...
    public void DrawSwirl()
    {
        ...
        Main.spriteBatch.EnterShaderRegion(ShaderRegistry.VortexShader.Effect, BlendState.Additive);
        const int amt = 30;
        for (int i = 0; i < amt; i++)
        {
            ...
            Color color = MulticolorLerp(completion, Color.Violet, Color.BlueViolet, Color.DarkViolet) *
                          Projectile.Opacity;
            Vector2 drawPosition = Projectile.oldPos[i] + Projectile.Size * 0.5f - Main.screenPosition;
            Main.spriteBatch.Draw(worleyNoise, drawPosition, null, color, spinRotation + (completion * MathHelper.Pi),
                worleyNoise.Size() * 0.5f, scale, 0, 0f);
        }
    ...
    public void DrawParticles()
    {
        ...
        const float energyBaseScale = 1f;

        foreach (EnergySuckParticle particle in Particles)
        {
            const float squish = 0.21f;
            ...
            Main.spriteBatch.Draw(bloom, drawPosition, null,
                particle.DrawColor * particle.Opacity * Projectile.Opacity * 0.8f, rotation, origin, scale * 0.32f, 0,
                0f);
            Main.spriteBatch.Draw(bloom, drawPosition, null, particle.DrawColor * particle.Opacity * Projectile.Opacity,
                rotation, origin, scale * 0.27f, 0, 0f);
            Main.spriteBatch.Draw(bloom, drawPosition, null, Color.White * particle.Opacity * Projectile.Opacity * 0.9f,
                rotation, origin, scale * 0.24f, 0, 0f);
        }
    }
}
