...
public class CollapsingStar : ModProjectile, IHasScreenShader
{
    ...
    {
        slot ??= LoopedSoundManager.CreateNew(
            new(AdditionsSound.sunAura, () => Utils.Remap(GrowTimer, 0f, GrowTime, 0f, 1.1f)),
        ...
        if ((!available || gen.Phase == TheExingendies.States.ActiveGalacticNucleus ||
             ModdedOwner.MouseRight.JustPressed) && FadeTimer <= 0f)
        {
        ...
        else if (FadeTimer > 0f)
        {
            ...
            if (available)
            {
                ...
                Projectile.Center = Vector2.SmoothStep(Projectile.Center, dest,
                    Animators.CubicBezier(.59f, .03f, .03f, .95f)(comp));
            }
            else
        ...
        {
            if (this.RunLocal() && AdditionsKeybinds.MiscHotKey.Current && AdditionsKeybinds.SetBonusHotKey.Current &&
                Projectile.scale >= IdealScale)
            {
            ...
            if (this.RunLocal())
            {
                ...
                Projectile.velocity = Vector2.Lerp(Projectile.velocity,
                    Projectile.SafeDirectionTo(dest) * MathF.Min(Projectile.Distance(dest), 20f), 0.07f);
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        ParticleRegistry.SpawnChromaticAberration(Projectile.Center, 50, .4f, KilonovaShockwave.MaxRadius / 2f);
        ...
        for (int i = 1; i <= amt; i++)
        {
            ...
            Color col = MulticolorLerp(lerp, Color.White, Color.White.Lerp(Color.Cyan, .5f), Color.Cyan, Color.SkyBlue,
                Color.DeepSkyBlue);
            Color col2 = col * 1.8f;
            ...
            if (i < blastAmt)
            {
                ...
                float blastScale = Utils.MultiLerp(blastComp, maxRad / 4f, maxRad / 3f, maxRad / 2f, maxRad);
                ParticleRegistry.SpawnDetailedBlastParticle(pos, Vector2.Zero, new(blastScale), Vector2.Zero, blastLife,
                    col * .6f);
            }

            ParticleRegistry.SpawnGlowParticle(pos, Main.rand.NextVector2Circular(100f, 100f), Main.rand.Next(90, 120),
                Main.rand.NextFloat(90f, 120f), col, Main.rand.NextFloat(1f, 2f));
            ParticleRegistry.SpawnGlowParticle(pos, Main.rand.NextVector2Circular(60f, 60f), Main.rand.Next(150, 180),
                Main.rand.NextFloat(90f, 100f), col, Main.rand.NextFloat(2f, 4f));
            ParticleRegistry.SpawnSquishyLightParticle(pos, vel, Main.rand.Next(40, 60), Main.rand.NextFloat(.6f, 1.4f),
                col2, 2f, 1.4f);

            ParticleRegistry.SpawnHeavySmokeParticle(pos, Main.rand.NextVector2Circular(20f, 20f),
                Main.rand.Next(40, 50), Main.rand.NextFloat(2f, 5f), col2.Lerp(col, .5f), 1.4f, true,
                Main.rand.NextFloat(-.1f, .1f));

            ParticleRegistry.SpawnCloudParticle(pos, vel * Main.rand.NextFloat(.5f, .9f), col, Color.Transparent,
                Main.rand.Next(120, 230), Main.rand.NextFloat(.8f, 1.5f), Main.rand.NextFloat(.8f, 1.6f),
                Main.rand.NextByte(0, 2));
            ParticleRegistry.SpawnCloudParticle(pos, vel * Main.rand.NextFloat(1.2f, 1.5f), col, Color.Transparent,
                Main.rand.Next(120, 230), Main.rand.NextFloat(.8f, 1.5f), Main.rand.NextFloat(.8f, 1.6f),
                Main.rand.NextByte(0, 2));

            ParticleRegistry.SpawnSparkParticle(pos, vel * 5f, Main.rand.Next(30, 40), Main.rand.NextFloat(5f, 8f),
                col2);
            ParticleRegistry.SpawnBloomLineParticle(pos, vel * 4f, Main.rand.Next(20, 30),
                Main.rand.NextFloat(1.8f, 5f), col2);
        }
    ...
    public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
    {
        foreach (SolarFlare flare in Flares)
        {
            ...
            if (targetHitbox.CollisionFromPoints(flare.Points.Points, c => flare.Trail.widthFunction(c)))
                return null;
        }
    ...
    public sealed class SolarFlare(float offset, Vector2 dir, int lifetime, float distance)
    {
        ...
        /// </summary>
        public readonly int Lifetime = lifetime;
    ...
    public void UpdateFlares()
    {
        if (OverallTimer % 35 == 34 && !IsCollapsing && FadeTimer <= 0f)
        {
            ...
            else
            {
                ...
                Flares.Add(new(rand, PolarVector(1f, rot).RotatedByRandom(.7f), Main.rand.Next(50, 80),
                    Main.rand.NextFloat(320f, 850f)));
            }
        }
        ...
        if (Flares.Count != 0)
        {
            foreach (SolarFlare flare in Flares)
            {
                ...
                {
                    SolarFlare flare1 = flare;
                    flare.Trail = new(c =>
                        {
                            float comp = 1f - InverseLerp(0f, flare1.Lifetime, flare1.Time);
                            return 25f * comp;
                        },
                        (c, pos) =>
                        {
                            float inter = InverseLerp(0f, flare.Lifetime, flare.Time);
                            return (FlareColor().Lerp(Color.DarkOrange, inter) * Animators.MakePoly(3f)
                                                                                   .OutFunction(1f - inter)
                                                                               * GetLerpBump(0f, .1f, 1f, .9f, c.X) *
                                                                               InverseLerp(0f, .1f, inter)) * .4f;
                        },
                        null, 50);
                }
                ...
                flare.B = PolarVector(rad, rot) + flare.Direction *
                    Animators.MakePoly(2.6f).OutFunction.Evaluate(0f, flare.Distance, interpolant);
