...
public class Myceliumite : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        switch (Variant)
        {
                ...
                if (Time > 15f)
                {
                    if (NPCTargeting.TryGetClosestNPC(new(Projectile.Center, 600, true, true), out NPC target))
                        Projectile.velocity = Vector2.Lerp(Projectile.velocity,
                            Projectile.SafeDirectionTo(target.Center) * 12f, .18f);
                }
            ...
            case MyceliumType.Piercing:
                ...
                if (Time > NeededTime)
                    Projectile.velocity =
                        Projectile.velocity.RotatedBy(.15f * (Projectile.identity % 2f == 0f).ToDirectionInt());
            ...
            case MyceliumType.Homing:
                for (int i = 0; i < 25; i++)
                    Dust.NewDustPerfect(Projectile.RandAreaInEntity(), DustID.GlowingMushroom,
                        Projectile.velocity * Main.rand.NextFloat(.5f, .7f), 0, default, Main.rand.NextFloat(.4f, .5f));
                break;
            case MyceliumType.Bouncy:
                for (int i = 0; i < 16; i++)
                    Dust.NewDustPerfect(Projectile.RandAreaInEntity(), DustID.MushroomTorch,
                        Projectile.velocity * Main.rand.NextFloat(.6f, .9f), 0, default, Main.rand.NextFloat(.6f, .8f));
                break;
            case MyceliumType.Piercing:
                for (int i = 0; i < Projectile.oldPos.Length; i++)
                {
                    for (int j = 0; j < 3; j++)
                        Dust.NewDustPerfect(Projectile.oldPos[i], DustID.MushroomTorch,
                            Projectile.velocity * Main.rand.NextFloat(.6f, .9f), 0, default,
                ...
                for (int i = 0; i < Main.rand.Next(30, 40); i++)
                {
                    if (i % 2f == 1f)
                        ParticleRegistry.SpawnGlowParticle(Projectile.Center,
                            Main.rand.NextVector2CircularLimited(9f, 9f, .5f, 1f), Main.rand.Next(40, 60), 1f,
                            Color.DodgerBlue);

                    Dust.NewDustPerfect(Projectile.RandAreaInEntity(), DustID.GlowingMushroom,
                        Main.rand.NextVector2CircularLimited(14f, 14f, .5f, 1f), 0, default,
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        {
            case MyceliumType.Homing:
                after?.DrawFancyAfterimages(tex, [new(32, 31, 110), new(34, 87, 142), new(30, 100, 172)],
                    Projectile.Opacity);
