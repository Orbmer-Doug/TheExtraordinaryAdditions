...
public class VolatileStar : ModProjectile, ILocalizedModType, IModType
{
    ...
    public Color CurrentColor => Color.Lerp(Color.White, MulticolorLerp(
                                     Cos01(Main.GlobalTimeWrappedHourly * 2f + Projectile.identity * .2f),
                                     Color.Violet, Color.DarkViolet, Color.BlueViolet, Color.SlateBlue,
                                     Color.DarkSlateBlue), .7f) *
                                 Projectile.Opacity * 1.1f;

    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        for (int i = 0; i < 14; i++)
        {
            Vector2 vel = -Projectile.velocity.RotatedByRandom(.2f).SafeNormalize(Vector2.Zero) *
                          Main.rand.NextFloat(2f, 7f);
            ParticleRegistry.SpawnGlowParticle(Projectile.Center, vel, Main.rand.Next(54, 90),
                Main.rand.NextFloat(.2f, .4f), CurrentColor);

            Dust.NewDustPerfect(Projectile.Center, DustID.AncientLight,
                vel.RotatedByRandom(.4f) * Main.rand.NextFloat(2.2f, 3f), Main.rand.Next(0, 200), CurrentColor,
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        Projectile.CreateFriendlyExplosion(Projectile.Center, Projectile.Size * 1.8f, Projectile.damage / 2,
            Projectile.knockBack / 2f, 20, 20);
