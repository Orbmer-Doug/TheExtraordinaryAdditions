...
public class CoalescedAmalgam : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Time % 2f == 1f)
        {
            ParticleRegistry.SpawnLightningArcParticle(Projectile.Center,
                Main.rand.NextVector2CircularLimited(180f, 180f, .5f, 1f), Main.rand.Next(6, 8),
                Main.rand.NextFloat(.6f, .8f) * Projectile.scale, Color.Gold);
        }
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            ParticleRegistry.SpawnHeavySmokeParticle(Projectile.Center, vel, Main.rand.Next(14, 23),
                Main.rand.NextFloat(.7f, 1f) * Projectile.scale, fireColor, 1f, true, angularVelocity);
        }
        ...
        if (Time < TimeToHome)
        {
            ...
            Projectile.velocity =
                Projectile.velocity.RotatedBy(.08f * (1f - Projectile.scale) *
                                              (Projectile.identity % 2 == 1).ToDirectionInt());
        ...
        if (Time == TimeToHome)
        {
            for (int i = 0; i < 30; i++)
                ParticleRegistry.SpawnSquishyLightParticle(Projectile.Center, Main.rand.NextVector2Circular(9f, 9f),
                    Main.rand.Next(20, 30), Main.rand.NextFloat(.3f, .5f), Color.LightGoldenrodYellow,
        ...
        if (Time > TimeToHome + 20f)
        {
            ...
            {
                float interpol = Animators.MakePoly(4)
                    .OutFunction(InverseLerp(TimeToHome + 20f, TimeToHome + 50f, Time));
                float speed = 26f * interpol;
                Projectile.velocity = Vector2.SmoothStep(Projectile.velocity,
                    Projectile.SafeDirectionTo(Target.Center) * speed, interpol * .2f);
            }
        }
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (int i = 0; i < 60; i++)
        {
            ...
            Vector2 vel = i < 30
                ? -Projectile.velocity.RotatedByRandom(.3f) * Main.rand.NextFloat(.4f, .9f)
                : -Projectile.velocity * Main.rand.NextFloat(.2f, .8f);
            ...
            {
                if (i % 4 == 3)
                    ParticleRegistry.SpawnLightningArcParticle(pos, vel.RotatedByRandom(.8f) * 6f, life - 10, scale,
                        col);
