...
public class TechnicBlitzripperProj : BaseIdleHoldoutProjectile, ILocalizedModType, IModType
{
    ...
    public Vector2 Tip => Projectile.Center + PolarVector(105f, Projectile.rotation) +
                          PolarVector(3f * Dir * Owner.gravDir, Projectile.rotation - MathHelper.PiOver2);
    ...
    public override void SafeAI()
    {
        ...
        Projectile.Center = Center + PolarVector(36f - (Recoil * 4), Projectile.rotation) +
        ...
        Owner.SetBackHandBetter(Player.CompositeArmStretchAmount.Full,
            Projectile.rotation + (.3f * Dir * Owner.gravDir));
        ...
        if (Overheating)
        {
            ...
            ParticleRegistry.SpawnHeavySmokeParticle(Tip, -Vector2.UnitY.RotatedByRandom(.3f) * speed,
                Main.rand.Next(20, 30), scale, col, opac);
        ...
        if (this.RunLocal() && Modded.SafeMouseRight.Current && !Overheating && !Modded.SafeMouseLeft.Current)
        {
            if (SniperTimer >= FireSniper)
            {
                ...
                for (int i = 0; i < 12; i++)
                    ParticleRegistry.SpawnGlowParticle(Tip, Main.rand.NextVector2Circular(2f, 2f),
                        Main.rand.Next(6, 12), Main.rand.NextFloat(40f, 60f), Color.LightCyan, 1.4f);

                for (int i = 0; i < 30; i++)
                    ParticleRegistry.SpawnTechyHolosquareParticle(Tip,
                        Projectile.velocity.RotatedByRandom(.4f).RotatedByRandom(.1f) * Main.rand.NextFloat(11f, 22f),
                        Main.rand.Next(20, 30), Main.rand.NextFloat(1f, 2f), Color.Cyan);
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            Main.spriteBatch.Draw(glow, drawPosition + off, null,
                Color.Cyan with { A = 0 } * comp * (Overheating ? .9f : .7f), rotation, glow.Size() / 2,
                Projectile.scale, effects, 0f);
        }
