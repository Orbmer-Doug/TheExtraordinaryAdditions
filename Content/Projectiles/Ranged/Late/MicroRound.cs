    ...
    public override bool OnTileCollide(Vector2 oldVelocity)
    {
        for (int i = 0; i < 20; i++)
        {
            Vector2 pos = Projectile.Center + Projectile.velocity.SafeNormalize(Vector2.Zero) * Projectile.width * .5f +
                          Main.rand.NextVector2CircularLimited(5f, 5f, .7f, 1f);
            Vector2 vel = -Projectile.velocity.RotatedByRandom(.09f) * Main.rand.NextFloat(.05f, .1f);
            ParticleRegistry.SpawnGlowParticle(pos, vel, Main.rand.Next(10, 20), Main.rand.NextFloat(.4f, .8f),
                Color.Lerp(Color.DarkRed, Color.White, .3f));
