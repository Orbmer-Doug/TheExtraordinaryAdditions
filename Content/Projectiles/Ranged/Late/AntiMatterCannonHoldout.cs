...
public class AntiMatterCannonHoldout : BaseIdleHoldoutProjectile
{
    ...
    public Vector2 Tip => Projectile.Center + PolarVector(166f, Projectile.rotation) +
    ...
    public override void SafeAI()
    {
        ...
        if (Reloading)
        {
            ...
            {
                Vector2 pos = Projectile.Center - PolarVector(83f, Projectile.rotation) +
        ...
        if ((this.RunLocal() && Modded.SafeMouseLeft.Current) && Wait <= 0f && !Reloading)
        {
            ...
            for (int i = 0; i < 10; i++)
                ParticleRegistry.SpawnGlowParticle(Tip, Vector2.Zero, 10, Main.rand.NextFloat(90f, 120f),
                    Color.OrangeRed, 1.3f);
            for (int i = 0; i < 40; i++)
            {
                ParticleRegistry.SpawnBloomLineParticle(Tip, vel.RotatedByRandom(.4f) * Main.rand.NextFloat(14f, 30f),
                    Main.rand.Next(8, 12), Main.rand.NextFloat(.5f, .9f), Color.OrangeRed);
            }
