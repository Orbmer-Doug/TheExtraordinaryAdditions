...
public class CharringBarrageHoldout : BaseIdleHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        if (Wait <= 0 && (this.RunLocal() && Modded.SafeMouseLeft.Current) && TryUseAmmo(out int type, out float speed,
                out int dmg, out float kb, out int ammo))
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                Dust.NewDustPerfect(pos, DustID.Torch, vel.RotatedByRandom(.3f) * Main.rand.NextFloat(.4f, .6f), 0,
                    default, Main.rand.NextFloat(.6f, .9f));
                ParticleRegistry.SpawnSparkParticle(pos, vel.RotatedByRandom(.2f) * Main.rand.NextFloat(.5f, .8f),
