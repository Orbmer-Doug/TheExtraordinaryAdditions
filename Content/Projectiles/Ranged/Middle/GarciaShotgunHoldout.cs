    ...
    public override void SafeAI()
    {
        ...
        if (animProgress == shell)
        {
            if (!Taunting)
            {
                if (Main.netMode != NetmodeID.Server)
                {
                    ...
                    Vector2 pos = Projectile.Center + PolarVector(14f, Projectile.rotation) +
                                  PolarVector(8f * Dir, Projectile.rotation - MathHelper.PiOver2);
                    Vector2 vel =
                        Projectile.rotation.ToRotationVector2().SafeNormalize(Vector2.Zero).RotatedBy(2f * -Dir) *
                ...
                out int dmg, out float kb, out int ammo))
        {
            Vector2 pos = Projectile.RotHitbox().Right +
                          PolarVector(2f * Dir, Projectile.rotation - MathHelper.PiOver2);
            ...
            for (int i = 0; i < 20; i++)
            {
                ParticleRegistry.SpawnSparkParticle(pos, vel.RotatedByRandom(.2f) * Main.rand.NextFloat(.6f, .8f),
                    Main.rand.Next(20, 30),
                    Main.rand.NextFloat(.7f, .8f), Color.Chocolate.Lerp(Color.OrangeRed, Main.rand.NextFloat(.4f, .5f)),
                    false, true);

                ParticleRegistry.SpawnGlowParticle(pos, Vector2.Zero, 10, Main.rand.NextFloat(.3f, .45f),
                    Color.OrangeRed, 1f);

                ParticleRegistry.SpawnMistParticle(pos,
                    vel.SafeNormalize(Vector2.Zero).RotatedByRandom(.45f) * Main.rand.NextFloat(1f, 4f),
                    Main.rand.NextFloat(.1f, .3f), Color.Chocolate * 1.2f, Color.DarkGray,
