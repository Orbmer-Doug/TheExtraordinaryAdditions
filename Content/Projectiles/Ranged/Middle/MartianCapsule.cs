...
{
    public override string Texture => AssetRegistry.GennedTextures.MartianCapsule.Path;
        ...
        if (Projectile.ai[0]++ > 15f)
        {
            if (NPCTargeting.TryGetClosestNPC(new(Projectile.Center, 450, true), out NPC target))
                Projectile.velocity = Vector2.SmoothStep(Projectile.velocity,
                    Projectile.SafeDirectionTo(target.Center) * 14f, .1f);
        }
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            Vector2 shootVelocity = (MathHelper.TwoPi * i / 4f + offsetAngle).ToRotationVector2() *
            ...
            for (int j = 0; j < 4; j++)
            {
                ParticleRegistry.SpawnSparkParticle(Projectile.Center,
                    Main.rand.NextVector2CircularLimited(3f, 3f, .5f, 1f),
                    Main.rand.Next(30, 50), Main.rand.NextFloat(.4f, .7f), Color.SkyBlue);
            }
        }

        ParticleRegistry.SpawnSparkleParticle(Projectile.Center, Vector2.Zero, Main.rand.Next(19, 23), 2.5f,
            Color.DeepSkyBlue, Color.LightCyan, 1.4f);
        ParticleRegistry.SpawnDetailedBlastParticle(Projectile.Center, Vector2.Zero, Vector2.One * 80f, Vector2.Zero,
            Main.rand.Next(16, 20), Color.Cyan);
    }
    ...
    {
        after?.DrawFancyAfterimages(Projectile.ThisProjectileTexture(), [Color.LightCyan, Color.Cyan, Color.DarkCyan],
            Projectile.Opacity);
