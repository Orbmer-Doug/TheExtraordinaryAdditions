...
public class CharringBlastBlast : ModProjectile, ILocalizedModType, IModType
{
    ...
    public override void AI()
    {
        if (Projectile.localAI[0] == 0f)
        {
            ...
            ParticleRegistry.SpawnDetailedBlastParticle(pos, Vector2.Zero, Projectile.Size, Vector2.Zero, 40,
                Color.OrangeRed, null, Color.DarkRed * .5f, true);
            for (int i = 0; i < 120; i++)
            {
                Vector2 dustSpawnOffset = Main.rand.NextVector2Unit(0f, MathHelper.TwoPi) *
                                          (float) Math.Pow(Main.rand.NextFloat(), 2.4) * Projectile.Size * 0.5f;
                Vector2 vel = dustSpawnOffset.SafeNormalize(Vector2.UnitY)
                ...
                ParticleRegistry.SpawnMistParticle(pos, vel, scale, Color.OrangeRed, Color.DarkRed,
                    Main.rand.NextByte(100, 150));

                if (Main.rand.NextBool(3))
                {
                    ParticleRegistry.SpawnSquishyPixelParticle(pos, vel * 2.5f, life * 6, scale * 7.4f, Color.OrangeRed,
                        Color.Chocolate * 1.4f, 8, true, true);
                }
            }
