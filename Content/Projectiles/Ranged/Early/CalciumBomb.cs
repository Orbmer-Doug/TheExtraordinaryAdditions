...
public class CalciumBomb : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        after.UpdateFancyAfterimages(new(Projectile.Center, Vector2.One, Projectile.Opacity * .4f, Projectile.rotation,
            0, 230, 0, 0f, null, false, -.2f));

        foreach (Projectile p in Main.ActiveProjectiles)
        {
            if (p != null && p.RotHitbox().Intersects(Projectile.RotHitbox()) &&
                p.type == ModContent.ProjectileType<CalciumShot>())
            {
                for (int i = 0; i < 8; i++)
                {
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        {
            if (i % 4 == 3)
                ParticleRegistry.SpawnDetailedBlastParticle(Projectile.Center, from, to * Main.rand.NextFloat(.5f, 1f),
                    Vector2.Zero, 40, Color.OrangeRed, RandomRotation(), null, true);
            ...
            ParticleRegistry.SpawnSquishyPixelParticle(Projectile.Center, vel, life, scale, col, Color.Chocolate, 3,
                true);
        }
    }
