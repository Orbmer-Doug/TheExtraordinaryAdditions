...
public class CrystallineBlast : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (((Proj is null || Proj.active == false) && notReady) || Owner.dead || Owner.active == false ||
            Projectile.penetrate <= 0 || (Released && Projectile.scale <= .2f))
        {
            Projectile.Kill();
            return;
        }
                ...
                if (this.RunLocal())
                {
                    ...
                    for (int i = 0; i < 12; i++)
                    {
                        ParticleRegistry.SpawnSparkParticle(Projectile.Center,
                            Projectile.velocity.RotatedByRandom(.35f) * Main.rand.NextFloat(.4f, .7f),
        ...
        if (Released)
        {
            ParticleRegistry.SpawnSquishyPixelParticle(Projectile.RotHitbox().RandomPoint(),
                -Projectile.velocity.SafeNormalize(Vector2.Zero) * Main.rand.NextFloat(2f, 4f) * Projectile.scale * 2f,
                Main.rand.Next(90, 120), Main.rand.NextFloat(.6f, .7f) * Projectile.scale * 2f, Color.Wheat,
                Color.White);
        }
    ...
    public override void OnKill(int timeLeft)
    {
        for (int i = 0; i < 10; i++)
        {
            ParticleRegistry.SpawnSquishyLightParticle(Projectile.RotHitbox().RandomPoint(),
                Main.rand.NextVector2Circular(4f, 4f) * Projectile.scale * 2f,
    ...
    private void OnHitEffects()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            Vector2 shootVelocity = (MathHelper.TwoPi * i / count + offsetAngle).ToRotationVector2() * 5f *
                                    InverseLerp(0f, .5f, Projectile.scale);
            ParticleRegistry.SpawnSparkParticle(pos, shootVelocity,
    ...
    public override bool OnTileCollide(Vector2 lastVelocity)
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            ParticleRegistry.SpawnGlowParticle(Projectile.Center, vel, life, size,
