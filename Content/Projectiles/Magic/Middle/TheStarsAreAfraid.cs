...
public class TheStarsAreAfraid : ModProjectile
{
    ...
    public float ChargeCompletion => Animators.MakePoly(3).OutFunction(InverseLerp(0f, ChargeTime, Time)) *
                                     (1f - InverseLerp(0f, 30f, ReleaseTime));
    ...
    public override void AI()
    {
        ...
        if (Owner == null || !Owner.active || ((ownerProj == null || !ownerProj.active) && !Released) ||
            !Main.bloodMoon)
        {
            Projectile.Kill();
            return;
        ...
        if (tele == null || tele.Disposed)
            tele = new((c) => Projectile.height / 4 * ChargeCompletion,
                (c, pos) => Color.Crimson * MathHelper.SmoothStep(1f, 0f, c.X) * ChargeCompletion, null, 20);
        ...
        if (Released)
        {
            if (ReleaseTime == 0f)
            {
                ...
                ParticleRegistry.SpawnDetailedBlastParticle(pos, Vector2.Zero, new Vector2(.4f, 1f) * 50f, Vector2.Zero,
                    40, Color.Crimson, Projectile.rotation);

                for (int i = 0; i < 8; i++)
                    ParticleRegistry.SpawnBloomLineParticle(pos,
                        Projectile.velocity.RotatedByRandom(.35f) * Main.rand.NextFloat(2f, 10f),
                        Main.rand.Next(40, 50), Main.rand.NextFloat(.4f, .6f), Color.Crimson);

                for (int i = 0; i < 14; i++)
                    ParticleRegistry.SpawnBloomPixelParticle(pos,
                        Projectile.velocity.RotatedByRandom(.45f) * Main.rand.NextFloat(4f, 14f),
                        Main.rand.Next(30, 50), Main.rand.NextFloat(.5f, .9f), Color.Crimson, Color.DarkRed, null, 1.2f,
                        4);
        ...
        else
        {
            ...
            Projectile.Center = Owner.RotatedRelativePoint(Owner.MountedCenter, false, true)
                                + PolarVector(300f * ChargeCompletion,
                                    (MathHelper.TwoPi * AmtCompletion) + ownerProj.AdditionsInfo().ExtraAI[0]);

            teleCache ??= new(20);
            teleCache.SetPoints(Projectile.Center.GetLaserControlPoints(
                Projectile.Center + Projectile.velocity.SafeNormalize(Vector2.Zero) * 500f * ChargeCompletion, 20));
            ...
            if (Time % 3 == 2)
            {
                Vector2 pos = Vector2.Lerp(Projectile.RotHitbox().Left, Projectile.RotHitbox().Right,
                    Main.rand.NextFloat());
                ...
                ParticleRegistry.SpawnCloudParticle(pos, Main.rand.NextVector2Circular(3f, 3f) + Owner.velocity,
                    Color.Crimson, Color.DarkRed, life, scale, opac, 2);
            }
        }
    ...
    {
        if (target.type == ModContent.NPCType<StygainHeart>() ||
            target.type >= NPCID.MoonLordHead && target.type <= NPCID.MoonLordLeechBlob)
        {
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            Color color = Color.DarkRed.Lerp(Color.Crimson, Main.rand.NextFloat(.4f, .6f)) *
                          Main.rand.NextFloat(.75f, 1f);
    ...
    internal float WidthFunction(float completionRatio)
    {
        return Projectile.height *
               MathHelper.SmoothStep(0.6f, 1f, Utils.GetLerpValue(0f, 0.3f, completionRatio, true)) * FadeOut;
    }
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Main.spriteBatch.Draw(texture, drawPosition, null, lightColor * Projectile.Opacity, Projectile.rotation, origin,
