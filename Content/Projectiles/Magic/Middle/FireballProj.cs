...
public class FireballProj : ModProjectile
{
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (i.BetweenNum(0, 2))
            {
                ...
                if (Main.rand.NextBool())
                    ParticleRegistry.SpawnSparkParticle(pos, -vel.RotatedByRandom(.4f), life / 2, scale, color, true,
                        true);
            }

            if (i == 0 && Main.rand.NextBool())
                ParticleRegistry.SpawnBloomPixelParticle(pos, -vel.RotatedByRandom(.17f), life, scale * .7f, color,
                    Color.White, null, 1.4f);
        }
    ...
    public void Kaboom()
    {
        ...
        for (int i = 0; i < 35; i++)
        {
            Vector2 vel = (MathHelper.TwoPi * i / 30 + RandomRotation()).ToRotationVector2() *
                          Main.rand.NextFloat(.5f, 7f) * Projectile.scale;
            int life = Main.rand.Next(30, 50);
            float scale = Main.rand.NextFloat(.5f, 1f) * Projectile.scale;
            Color color = FireballHoldout.FireColors[Main.rand.Next(FireballHoldout.FireColors.Length - 1)];

            ParticleRegistry.SpawnHeavySmokeParticle(pos, vel, life, scale, color);
            ParticleRegistry.SpawnCloudParticle(pos, vel * .4f, color, Color.DarkGray, life, scale * 60f,
                Main.rand.NextFloat(.5f, .8f));
            ParticleRegistry.SpawnGlowParticle(pos, vel, life, scale * 150, color, .4f);
            ParticleRegistry.SpawnSparkParticle(pos, vel * 3.4f + Vector2.UnitY * -5f, life / 2, scale * 1.8f,
                color * 1.2f);
        }

        SoundEngine.PlaySound(SoundID.Item14 with { Pitch = -.2f, Volume = Projectile.scale + .1f, MaxInstances = 0 },
            Projectile.Center);
        Projectile.CreateFriendlyExplosion(Projectile.Center, Projectile.Size * 3.5f, Projectile.damage,
            Projectile.knockBack, 5, 7);
    }
