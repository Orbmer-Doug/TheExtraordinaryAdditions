...
public class CometStormHoldout : BaseHoldoutProjectile
{
    ...
    public override void SafeAI()
    {
        ...
        Vector2 top = Projectile.Center + PolarVector(Projectile.height / 2, Projectile.velocity.ToRotation()) +
                      PolarVector(16f, Projectile.velocity.ToRotation() - MathHelper.PiOver2);
        ...
        if (Time % 7f == 6f)
        {
            ...
            if (this.RunLocal() && TryUseMana())
            {
                for (int i = 0; i < 2; i++)
                {
                    Vector2 position = Owner.Center -
                                       new Vector2(
                                           Main.rand.NextFloat(-Main.LogicCheckScreenWidth / 4,
                                               Main.LogicCheckScreenWidth / 4),
                                           800f + (Main.rand.NextFloat(20f, 50) * i));
            ...
            ParticleRegistry.SpawnPulseRingParticle(top, Vector2.Zero, 20, RandomRotation(),
                new(Main.rand.NextFloat(.3f, .7f), 1f), 0f, Main.rand.NextFloat(120f, 180f), Color.Cyan);
        }

        ParticleRegistry.SpawnBloomPixelParticle(top + Main.rand.NextVector2Circular(100f, 100f),
            RandomVelocity(2f, 4f, 10f), Main.rand.Next(18, 24),
