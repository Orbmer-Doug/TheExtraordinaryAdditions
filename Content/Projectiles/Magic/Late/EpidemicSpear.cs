...
public class EpidemicSpear : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (this.RunLocal() && Owner.Additions().MouseRight.Current && !Released)
        {
            ...
            Projectile.velocity = Vector2.Lerp(Projectile.velocity,
                Projectile.SafeDirectionTo(Proj.Center + Vector2.UnitY * -(Proj.Size.Length() * .5f + 80f)) *
        ...
        for (int x = -1; x <= 1; x += 2)
        {
            ...
            {
                Vector2 vel = (MathHelper.TwoPi * i / arms + Offset).ToRotationVector2() * Main.rand.NextFloat(2f, 4f) *
                              Projectile.scale;
                if (x == -1)
                    vel = (MathHelper.TwoPi * i / arms - Offset).ToRotationVector2() * Main.rand.NextFloat(4f, 8f) *
                          Projectile.scale;

                ShaderParticleRegistry.SpawnEpidemicParticle(Projectile.Center, vel,
                    x == -1 ? 45f : 40f * Projectile.scale);
            }
        }
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        for (int i = 0; i < 12; i++)
        {
            ParticleRegistry.SpawnCloudParticle(Projectile.Center,
                Projectile.velocity.RotatedByRandom(.2f) * Main.rand.NextFloat(.4f, .6f),
                Color.LimeGreen, Color.DarkOliveGreen, Main.rand.Next(20, 50), Main.rand.NextFloat(.5f, .7f),
