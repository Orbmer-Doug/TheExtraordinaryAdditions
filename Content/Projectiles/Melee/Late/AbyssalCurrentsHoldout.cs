...
public class AbyssalCurrentsHoldout : ModProjectile, IHasScreenShader
{
    ...
    public void DoReel()
    {
        ...
        {
            Projectile.velocity =
                Vector2.SmoothStep(Projectile.velocity, Center.SafeDirectionTo(Modded.MouseWorld), .6f);
        ...
        float reelAnim = Animators.MakePoly(3f).InOutFunction
            ...
            for (int j = 0; j < 2; j++)
            {
                Vector2 vel =
                    Projectile.velocity.SafeNormalize(Vector2.Zero)
                        .RotatedBy(i == 0 ? -.6f : i == 1 ? -.4f : i == 2 ? .4f : .6f) * -(j == 0 ? 8f : 15f);
    ...
    public void DoSpin()
    {
        ...
        if (rand.NextBool(3))
        {
            Vector2 pos = Projectile.Center + rand.NextVector2CircularLimited(200f, 200f, .6f, 1f);
            Vector2 vel = pos.SafeDirectionTo(Projectile.Center);
            ParticleRegistry.SpawnDustParticle(pos, vel, rand.Next(40, 60), rand.NextFloat(.6f, 1f),
                CoreUtils.MulticolorLerp(rand.NextFloat(), AbyssalCurrents.WaterPalette), .1f, false, true, true);
        }

        Owner.SetFrontHandBetter(Player.CompositeArmStretchAmount.Full, Projectile.velocity.ToRotation());
        Projectile.rotation += Lerp(0f, .4f, completion) * Projectile.velocity.X.NonZeroSign();
    ...
    public void DoChase()
    {
        ...
        water ??= LoopedSoundManager.CreateNew(new(AdditionsSound.waterfall, () => completion * 4.7f),
            () => AdditionsLoopedSound.ProjectileNotActive(Projectile), () => State == AbyssalState.Chase);
        water?.Update(Projectile.Center);

        foreach (NPC npc in ActiveNPCs)
        {
            if (npc.CanHomeInto() && !npc.boss && npc.IsAnEnemy() && npc.realLife <= 0 && npc.velocity.Length() > 1f &&
                npc.knockBackResist != 0f && !npc.dontTakeDamage)
            {
                npc.velocity += npc.SafeDirectionTo(Projectile.Center) *
                                GaussianFalloff2D(Projectile.Center, npc.Center, .2f * npc.knockBackResist, 1000f);
            }
        }
        ...
        if (this.RunLocal() && Modded.MouseRight.Current && BackTime <= 0)
        {
            ...
            Vector2 force = Projectile.Center.SafeDirectionTo(dest) *
                            (MathF.Min(Projectile.Distance(dest), 15f) * (away ? 1.8f : 1f));
        ...
        else if (this.RunLocal() && (!Modded.MouseRight.Current || BackTime > 0))
        {
            if (BackTime >= WhirlpoolRecede)
            {
                Projectile.velocity = Vector2.SmoothStep(Projectile.velocity,
                    Projectile.SafeDirectionTo(Owner.Center) * Clamp(Projectile.Distance(Owner.Center), 5f, 50f), .2f);
                Projectile.rotation = Projectile.rotation.AngleLerp(Projectile.velocity.ToRotation() + PiOver4, .4f);
    ...
    public override void CutTiles()
    {
        ...
        Utils.PlotTileLine(Projectile.RotHitbox().BottomLeft, Projectile.RotHitbox().TopRight, 130f,
    ...
    public void UpdateShader()
    {
        ...
        Shader.TrySetParameter("distortionPosition",
            Vector2.Transform(Projectile.Center - screenPosition, GameViewMatrix.TransformationMatrix));
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        if (State == AbyssalState.Spin || State == AbyssalState.Chase)
        {
            ...
            float area = Lerp(14f, 2f, comp);
            Projectile.DrawProjectileBackglow(AbyssalCurrents.WaterPalette[2] * comp * opac, area, 0, 4);
        }

        spriteBatch.DrawBetter(tex, Projectile.Center, null, Color.White * opac, Projectile.rotation, orig,
            Projectile.scale);
        if (State == AbyssalState.Reel)
        {
            ...
            float fade = State == AbyssalState.Throw
                ? 1f
                : Animators.MakePoly(2f).OutFunction(InverseLerp(0f, ReelTime, Time));
        ...
        if (State == AbyssalState.Chase)
        {
            ...
            void pool()
            {
                ...
                float completion = InverseLerp(0f, WhirlpoolGrow, Time) * InverseLerp(WhirlpoolRecede, 0f, BackTime);
                spriteBatch.Draw(noise, ToTarget(Projectile.Center, new Vector2(400f) * completion), null,
                    Color.Cyan * completion, 0f, noise.Size() / 2f, 0, 0f);
            }
