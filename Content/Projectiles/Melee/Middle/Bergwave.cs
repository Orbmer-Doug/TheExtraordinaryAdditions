...
public class Bergwave : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        foreach (Vector2 point in points)
        {
            ...
            {
                ParticleRegistry.SpawnCloudParticle(point, Projectile.velocity * .2f, AuroraGuard.Icey,
                    AuroraGuard.PastelViolet, Main.rand.Next(20, 30),
                    Main.rand.NextFloat(20f, 40f), Main.rand.NextFloat(.6f, 1f) * scaling, Main.rand.NextByte(0, 2));
            }

            if (Main.rand.NextBool(16))
                ParticleRegistry.SpawnBloomPixelParticle(point, -Projectile.velocity * Main.rand.NextFloat(.2f, .4f),
                    Main.rand.Next(30, 40), Main.rand.NextFloat(.4f, .6f) * scaling, AuroraGuard.MauveBright,
                    AuroraGuard.Lavender);
        }
    }
