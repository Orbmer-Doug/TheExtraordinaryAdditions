...
public class BergIcicle : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        {
            Projectile.rotation +=
                Projectile.velocity.X.NonZeroSign() * Terraria.Utils.Remap(Time, 0f, WaitTime, .28f, 0f);
        ...
        else
        {
            if (!Hit)
            {
                if (FindProjectile(out Projectile p, ModContent.ProjectileType<BergcrusherSwing>(), Projectile.owner))
                {
                    ...
                    if (swing.Rect().Intersects(Projectile.RotHitbox()) && swing.AngularVelocity > .03f &&
                        swing.Time > 5)
                    {
                        Hit = true;
            ...
            if (Hit)
            {
                ...
                if (Main.rand.NextBool(3))
                    ParticleRegistry.SpawnBloomPixelParticle(Projectile.RotHitbox().RandomPoint(),
                        -Projectile.velocity * .02f, Main.rand.Next(30, 40), Main.rand.NextFloat(.3f, .5f),
                        Color.SlateBlue, Color.LightBlue, null);
            }
        }
