...
public class KnifeStab : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        {
            Projectile.timeLeft =
                (int) (Owner.HeldItem.useAnimation * (1f / Owner.GetTotalAttackSpeed(Projectile.DamageType)));
        ...
        if (!Fading)
        {
            ...
            if (stabTimer < totalTime)
            {
                ...
                if (stabTimer < totalTime * 0.5f)
                {
                    ...
                    offset = Vector2.Lerp(new Vector2(dist, 0), stabVec, circ)
                        .RotatedBy(Projectile.rotation - MathHelper.PiOver4);
                }
                else
                {
                    if (!PlayedSound)
                    ...
                    float circ = Circ.OutFunction(lerper);
                    offset = Vector2.Lerp(stabVec, new Vector2(dist, 0), circ)
                        .RotatedBy(Projectile.rotation - MathHelper.PiOver4);
                }
            }
            else
    ...
    public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
    {
        return targetHitbox.LineCollision(Projectile.Center,
            Projectile.Center + Projectile.velocity.SafeNormalize(Vector2.Zero) * Projectile.width * .5f *
            Projectile.scale, 14f * Projectile.scale);
    }
    ...
    public override void CutTiles()
    {
        ...
        Vector2 end = Projectile.Center +
                      Projectile.velocity.SafeNormalize(Vector2.Zero) * Projectile.width * .5f * Projectile.scale;
        Terraria.Utils.PlotTileLine(start, end, 14f * Projectile.scale, DelegateMethods.CutTiles);
    }

    public readonly int[] ObliteratableTypes =
    [
        NPCID.CaveBat, NPCID.GiantBat, NPCID.IceBat, NPCID.IlluminantBat,
        NPCID.JungleBat, NPCID.SporeBat, NPCID.VampireBat, NPCID.Hellbat, NPCID.Lavabat, NPCID.Medusa,
    ...
    public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
    {
        ...
        for (int i = 0; i < 14; i++)
        {
            Vector2 vel = target.Center.SafeDirectionTo(Owner.Center).RotatedByRandom(.2f) *
                          Main.rand.NextFloat(1f, 12f);
            ...
            {
                if (i == 0)
                    ParticleRegistry.SpawnBloodStreakParticle(start, vel.SafeNormalize(Vector2.Zero), life * 2,
                        scale * .7f, Color.DarkRed);
                ParticleRegistry.SpawnBloodParticle(start, vel, life, scale,
                    Color.Crimson.Lerp(Color.Red, Main.rand.NextFloat(.25f, .78f)));
            }
            else
            {
                ParticleRegistry.SpawnSparkParticle(start, vel * Main.rand.NextFloat(1.2f, 2.1f), life, scale,
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Main.spriteBatch.Draw(tex, pos + off, null, Lighting.GetColor(Projectile.Center.ToTileCoordinates()) * fade,
            rot, tex.Size() / 2, scale, fx, 0);
        return false;
    }
}
