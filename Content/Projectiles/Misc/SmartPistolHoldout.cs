...
public class SmartPistolGlobalNPC : GlobalNPC
{
    ...
    {
        if (Utility.AnyProjectile(ModContent.ProjectileType<SmartPistolHeld>()))
        {
            int count = npc.boss ? 3 : 1;
            binaryWriter.Write((byte)count);
            for (int i = 0; i < count; i++)
            {
                binaryWriter.Write(LockIn[i]);
                if (npc.boss)
                    binaryWriter.WriteVector2(LockPositions[i]);
            }
        }
    }
    ...
    public override void ReceiveExtraAI(NPC npc, BitReader bitReader, BinaryReader binaryReader)
    {
        if (Utility.AnyProjectile(ModContent.ProjectileType<SmartPistolHeld>()))
        {
            ...
            for (int i = 0; i < count; i++)
            {
                LockIn[i] = binaryReader.ReadInt32();
                if (npc.boss && i < 3)
                    LockPositions[i] = binaryReader.ReadVector2();
            }
        }
    }
...
public class SmartPistolLayer : PlayerDrawLayer
{
    ...
    public override void Draw(ref PlayerDrawSet drawInfo)
    {
        ...
        Drawing = false;
        if (drawPlayer != null && Utility.FindItem(out Item item, drawPlayer, ModContent.ItemType<SmartPistolMK6>()) && drawInfo.shadow == 0f && Main.myPlayer == drawPlayer.whoAmI)
        {
        ...
        void draw()
        {
            ...
            foreach (NPC target in Targets)
            {
                ...
                for (int i = 0; i < c; i++)
                {
                    ...
                    ManualTrailPoints points = new(40);
