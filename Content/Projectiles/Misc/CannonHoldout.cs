...
public class CannonHoldout : ModProjectile
{
    ...
    public const float DrillLength = 90f;
        ...
        for (int x = destructleft; x <= destructright; x++)
        {
            for (int y = destructdown; y <= destructup; y++)
            {
                ...
                if (val.HasTile)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                        ParticleRegistry.SpawnSparkParticle(pos, Main.rand.NextVector2Circular(5f, 5f),
                            Main.rand.Next(20, 30),
                            Main.rand.NextFloat(.3f, .7f),
                            Color.OrangeRed.Lerp(Color.Red, Main.rand.NextFloat(0f, .3f)), true, true);
    ...
    public void DrawBeam(Texture2D beamTex, Vector2 direction, int beamIndex, bool second)
    {
        ...
        Vector2 beamScale = new(Scale, val2.Length() / beamTex.Height);
        val2 = default;
        ...
        if (second)
        {
            ...
            val2 = startPos - Projectile.Center;
            beamScale = new(Scale, val2.Length() / beamTex.Height);
            val2 = default;
        }

        DrawChromaticAberration(direction.RotatedBy(MathHelper.PiOver2, val2), 4f,
            delegate(Vector2 offset, Color colorMod)
            {
                ...
                Main.EntitySpriteDraw(beamTex, startPos + offset - Main.screenPosition, null, val3,
                    rotation + MathHelper.PiOver2, beamOrigin, beamScale, 0);
                beamScale.X = 2.4f;
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        Main.EntitySpriteDraw(tex, pos, null, Projectile.GetAlpha(lightColor), Projectile.rotation, origin,
            Projectile.scale, effect);
        ...
        for (int i = 0; i < 8; i++)
            Main.EntitySpriteDraw(bloom, pos + (MathHelper.TwoPi * i / 8).ToRotationVector2() * 2f, null,
                bloomColor * bloomOpacity, Projectile.rotation, origin, Projectile.scale, effect);
