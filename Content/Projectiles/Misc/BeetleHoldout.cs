...
public class BeetleHoldout : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        Vector2 rise = new(Owner.direction * Projectile.width * .6f,
            -30f * Animators.MakePoly(3f).InFunction(RiseInterpolant) * Owner.gravDir);
        ...
        Owner.SetFrontHandBetter(Player.CompositeArmStretchAmount.Full,
            center.SafeDirectionTo(Projectile.Center).ToRotation() * Owner.gravDir);
        Projectile.Opacity = Interpolant;

        if (Main.rand.NextBool((int) (10 * (1.2f - RiseInterpolant))))
            ParticleRegistry.SpawnCloudParticle(Projectile.RotHitbox().RandomPoint(),
                Main.rand.NextVector2Circular(2f, 2f), Color.Crimson, Color.DarkRed,
        ...
        if (Main.rand.NextBool((int) (5 * (1.2f - RiseInterpolant))))
            ParticleRegistry.SpawnBloomPixelParticle(Projectile.RotHitbox().RandomPoint(),
                Vector2.UnitY * -(Main.rand.NextFloat(2f, 5f) * Interpolant),
                Main.rand.Next(30, 45), Main.rand.NextFloat(.4f, .6f), Color.DarkRed, Color.Crimson * 1.4f, null, 1f,
                3);
        ...
        if (Time >= AnimTime)
        {
            ...
            if (Right)
            {
                if (this.RunLocal())
                {
                    if (!SuperBloodMoonSystem.SuperBloodMoon)
                    {
                        ...
                        DisplayText(
                            ModContent.GetModItem(ModContent.ItemType<CrimsonCarvedBeetle>())
                                .GetLocalizedValue("TurnOn"), Color.Crimson);
                    }
                    else
                    {
                        ...
                        DisplayText(
                            ModContent.GetModItem(ModContent.ItemType<CrimsonCarvedBeetle>())
                                .GetLocalizedValue("TurnOff"), Color.DarkRed);
                    }
            ...
            else
            {
                if (Main.netMode != NetmodeID.MultiplayerClient)
                    ...
                    NetMessage.SendData(MessageID.SpawnBossUseLicenseStartEvent, -1, -1, null, Owner.whoAmI,
                        ModContent.NPCType<StygainHeart>(), 0f, 0f, 0, 0, 0);
            }
