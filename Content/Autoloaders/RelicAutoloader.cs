...
{
    [Autoload(false)]
    public class AutoloadableRelicItem : ModItem
    {
        internal int tileID;
    ...
    public static void Create(Mod mod, string itemPath, string tilePath, out int itemID)
    {
        AutoloadableRelicItem relicItem = new(itemPath);
        mod.AddContent(relicItem);
        itemID = relicItem.Type;

        AutoloadableRelicTile relicTile = new(tilePath);
        mod.AddContent(relicTile);
        int tileID = relicTile.Type;
