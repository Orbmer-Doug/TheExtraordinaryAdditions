        ...
        else if (Time == AuroraGuard.TimeToRise)
        {
            ...
            for (int i = 0; i < 40; i++)
            {
                Vector2 pos = GroundPos +
                              Vector2.UnitX * Main.rand.NextFloat(-Projectile.width / 2f, Projectile.width / 2f) -
                              Vector2.UnitY * 8f;
                Vector2 vel = -pos.SafeDirectionTo(GroundPos) * Main.rand.NextFloat(6f, 11f);
                ParticleRegistry.SpawnDustParticle(pos, vel, Main.rand.Next(30, 60), Main.rand.NextFloat(.5f, 1.2f),
                    Color.Cyan, .1f, true, true);
                ParticleRegistry.SpawnBloomPixelParticle(pos, vel, Main.rand.Next(30, 60),
            ...
            if (ModProjectile.RunServer())
            {
                for (int i = 0; i < SnowflakeCount; i++)
                {
                    Vector2 pos = GroundPos +
                                  Vector2.UnitX * Main.rand.NextFloat(-Projectile.width / 2f, Projectile.width / 2f) -
                                  Vector2.UnitY * 8f;
                    Vector2 vel = -Vector2.UnitY.RotatedBy(
                        Convert01To101(InverseLerp(GroundPos.X - Projectile.width / 2f,
                            GroundPos.X + Projectile.width / 2f, pos.X))
                        * Utils.Remap(pos.X, GroundPos.X - Projectile.width / 2f, GroundPos.X + Projectile.width / 2f,
                            -.3f, .3f)) * Main.rand.NextFloat(16f, 21f);
