...
public class OverheatedBlast : ProjOwnedByNPC<AuroraGuard>
{
    ...
    public override void SafeAI()
    {
        if (Projectile.ai[0] == 0f)
        {
            ...
            for (int i = 0; i < 4; i++)
                ParticleRegistry.SpawnDetailedBlastParticle(pos, Vector2.Zero, Vector2.One * Projectile.width,
                    Vector2.Zero, 40 - (i * 4), SlateBlue, null, Icey, true);
            for (int i = 0; i < 100; i++)
            {
                ...
                ParticleRegistry.SpawnCloudParticle(pos, vel, col, DeepBlue, life * 2, scale * .6f,
                    Main.rand.NextFloat(.4f, .8f), Main.rand.NextByte(0, 2));
            }
