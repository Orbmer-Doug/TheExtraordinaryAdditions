...
public class SolarGuardian : ModNPC
{
    ...
    public override void SetDefaults()
    {
        ...
        NPC.aiStyle = -1;
        AIType = -1;
        NPC.value = Item.buyPrice(0, 6, 25);
    ...
    {
        if (!NPC.SearchForPlayerTarget(out Player target) && State != GuardianStates.Idle)
        {
            Time = 0;
        ...
        switch (State)
        {
            case GuardianStates.Idle:
                ...
                {
                    RotateDir = Main.rand.NextFromCollection([-1, 1]);
                    Time = 0;
            ...
            case GuardianStates.Shooting:
                ...
                NPC.velocity = Vector2.SmoothStep(NPC.velocity,
                    NPC.SafeDirectionTo(dest) * MathF.Min(NPC.Distance(dest), 17f), .2f);
                ...
                if (Time < ChargeTime)
                {
                    ParticleRegistry.SpawnDustParticle(NPC.RotHitbox().RandomPoint(),
                        -Vector2.UnitY * Main.rand.NextFloat(3f, 8f),
                        ...
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            for (int o = 0; o < 6; o++)
                            {
                                ParticleRegistry.SpawnBloomLineParticle(
                                    ...
                                    vel.RotatedByRandom(.4f) * Main.rand.NextFloat(.7f, 3f), Main.rand.Next(30, 50),
                                    Main.rand.NextFloat(.3f, .6f), Color.OrangeRed);
                                ParticleRegistry.SpawnBloomPixelParticle(
                                    NPC.Center + Main.rand.NextVector2Circular(20f, 20f),
                                    vel.RotatedByRandom(.3f) * Main.rand.NextFloat(.5f, 2.5f), Main.rand.Next(40, 60),
                                    Main.rand.NextFloat(.4f, .7f), Color.OrangeRed, Color.Orange, null, 1.1f, 3);
                            }
                        }
            ...
            case GuardianStates.Dash:
                if (Time < ReelBackTime)
                {
                    NPC.velocity = -NPC.SafeDirectionTo(target.Center) *
                                   Animators.MakePoly(4f).InFunction.Evaluate(Time, 0f, ReelBackTime, 7f, 1f);
                ...
                if (NPC.velocity.Length() > 9f)
                    ParticleRegistry.SpawnGlowParticle(NPC.RotHitbox().RandomPoint(),
                        -NPC.velocity * Main.rand.NextFloat(.2f, .4f), Main.rand.Next(30, 50),
                        Main.rand.NextFloat(40f, 90f), Color.OrangeRed, 1.2f);
    ...
    {
        if (!spawnInfo.Player.ZoneSkyHeight || spawnInfo.PlayerSafe || !Main.hardMode)
            return 0f;
    ...
    public override void HitEffect(NPC.HitInfo hit)
    {
        for (int j = 0; j < 3; j++)
        {
            Dust o = Dust.NewDustDirect(NPC.position, NPC.width, NPC.height, DustID.OrangeTorch, hit.HitDirection * 2,
                hit.HitDirection * 2, 0, default(Color), 3f);
            o.noGravity = true;
        }
    ...
    {
        npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<WrithingLight>()));
    }
    ...
    public override bool PreDraw(SpriteBatch spriteBatch, Vector2 screenPos, Color drawColor)
    {
        ...
        if (State == GuardianStates.Dash)
        {
            ...
            for (int i = 0; i < imageAmt; i++)
            {
                ...
                float opac = InverseLerp(imageAmt, 0, i) * .7f * InverseLerp(0f, 20f, Time) *
                             InverseLerp(ReelBackTime + DashTime + SlowdownTime, ReelBackTime + DashTime, Time);
                spriteBatch.DrawBetter(tex, pos, NPC.frame, Color.Orange with { A = 0 } * opac, rot, orig, 1f);
            }
        }

        spriteBatch.DrawBetter(tex, NPC.Center, NPC.frame, Color.White, NPC.rotation, orig, 1f);
        if (State == GuardianStates.Shooting)
        {
            if (Time < ChargeTime)
            {
                for (int i = 0; i < 6; i++)
                {
                    ...
                    spriteBatch.DrawBetter(tex, NPC.Center + offset, NPC.frame, Color.OrangeRed with { A = 0 } * opac,
                        NPC.rotation, orig, 1f);
                }
            }
        }
