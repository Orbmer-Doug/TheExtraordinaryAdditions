...
public class MassExplosion : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (Time == 0f)
        {
            ...
            for (int i = 0; i < 120; i++)
            {
                ParticleRegistry.SpawnBloomPixelParticle(pos, Main.rand.NextVector2CircularLimited(30f, 30f, .6f, 1f),
                    Main.rand.Next(32, 34), Main.rand.NextFloat(.6f, 1.2f), Color.Crimson, Color.DarkRed, null, 1.4f,
                    10, false, true);
                ParticleRegistry.SpawnGlowParticle(pos, Main.rand.NextVector2Circular(18f, 18f), Main.rand.Next(40, 70),
                    Main.rand.NextFloat(70f, 120f), Color.Crimson * 1.4f, .7f);
                ParticleRegistry.SpawnCloudParticle(pos, Main.rand.NextVector2Circular(4f, 4f), Color.Crimson,
                    Color.DarkRed, Main.rand.Next(40, 60), Main.rand.NextFloat(40f, 100f),
                    Main.rand.NextFloat(.6f, .8f), 1);
            }
