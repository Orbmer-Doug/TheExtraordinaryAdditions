...
public class TaintedStar : ProjOwnedByNPC<StygainHeart>
{
    ...
    public override void SafeAI()
    {
        ...
        {
            int starGore = Gore.NewGore(Projectile.GetSource_FromAI(), Projectile.Center,
                new Vector2(Projectile.velocity.X * 0.2f, Projectile.velocity.Y * 0.2f),
                Main.rand.NextBool() ? GoreID.BloodSquid3 : GoreID.BloodZombieChunk2);
    ...
    public override void OnHitPlayer(Player target, Player.HurtInfo info)
    {
        ...
        for (int i = 0; i <= 10; i++)
        {
            ParticleRegistry.SpawnBloodParticle(target.Center,
                (-Projectile.velocity * Main.rand.NextFloat(.4f, 1f)).RotatedByRandom(.5),
    ...
    public override void OnKill(int timeLeft)
    {
        ...
        if (Main.netMode != NetmodeID.Server)
        {
            for (int i = 0; i < 3; i++)
            {
                Gore.NewGore(Projectile.GetSource_Death(), Projectile.position,
