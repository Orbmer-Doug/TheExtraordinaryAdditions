...
public class BloodRay : ProjOwnedByNPC<StygainHeart>
{
    ...
    public override void SafeAI()
    {
        ...
        if (Utility.FindProjectile(out Projectile barrier, ModContent.ProjectileType<HemoglobBarrier>()))
        {
            if (Vector2.Distance(Projectile.Center, barrier.Center) >= StygainHeart.BarrierSize)
            {
                barrier.As<HemoglobBarrier>().CurrentWidth += .1f;
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        PixelationSystem.QueuePrimitiveRenderAction(draw, Core.Graphics.PixelationLayer.UnderProjectiles);
