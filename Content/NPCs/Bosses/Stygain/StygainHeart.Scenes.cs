...
{
    public void DoAttack_SpawnEffects(NPC npc, Player target, ref float attackTimer)
    {
        ...
        else
        {
            ...
            CombatText.NewText(target.Hitbox, Color.PaleVioletRed, "A storm brews...", true);
    ...
    public void DoBehavior_Phase2Drama(NPC npc, Player target, ref float attackTimer)
    {
        float totalTime = SecondsToFrames(7f);
    ...
    public void DoBehavior_DeathEffects(NPC npc, Player target, ref float attackTimer)
    {
        HideBossBar(npc);
        HasDoneBloodBeacon = false;
