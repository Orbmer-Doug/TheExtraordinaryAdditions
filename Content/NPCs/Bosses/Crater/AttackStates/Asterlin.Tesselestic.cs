...
public partial class Asterlin : ModNPC
{
    public static readonly Dictionary<AsterlinAIType, float> Tesselestic_PossibleStates =
        new Dictionary<AsterlinAIType, float>
            { { AsterlinAIType.Disintegration, 1f }, { AsterlinAIType.Lightripper, .6f } };
    ...
    public void LoadStateTransitions_Tesselestic()
    {
        StateMachine.RegisterTransition(AsterlinAIType.Tesselestic, Tesselestic_PossibleStates, false,
            () => Tesselestic_FadeTime > Tesselestic_FadeDuration);
    ...
    {
        Vector2 destination = new(Target.Center.X + 300f * (NPC.Center.X > Target.Center.X).ToDirectionInt(),
            Target.Center.Y - 140f);
        ...
        if (AITimer < Tesselestic_ChargeUp)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                ParticleRegistry.SpawnTechyHolosquareParticle(pos, pos.SafeDirectionTo(Staff.TipOfStaff) * 4f, life,
                    scale, Color.DeepSkyBlue);
            }
        }
