...
public partial class Asterlin
{
    public static readonly Dictionary<AsterlinAIType, float> Cleave_PossibleStates =
        new Dictionary<AsterlinAIType, float>
            { { AsterlinAIType.Hyperbeam, 1f }, { AsterlinAIType.TechnicBombBarrage, .6f } };
    ...
    public void DoBehavior_Cleave()
    {
        ...
        {
            NPC.SmoothFlyNear(
                new Vector2(Target.Center.X + 100f * (NPC.Center.X > Target.Center.X).ToDirectionInt(),
                    Target.Center.Y - 20f), .15f, .9f);
        ...
        else
        {
            ...
            if (!Cleave_HitGround)
            {
                ...
                if (RotatedHitbox.SolidCollision())
                {
                    ...
                    for (int i = 0; i < 120; i++)
                    {
                        ParticleRegistry.SpawnSquishyPixelParticle(ground,
                            -Vector2.UnitY.RotatedByRandom(MathHelper.PiOver2) * Main.rand.NextFloat(8f, 70f),
                            Main.rand.Next(200, 310), Main.rand.NextFloat(2.7f, 4.8f), Color.Gold, Color.OrangeRed, 8,
                            false, true);
                        ParticleRegistry.SpawnMistParticle(ground,
                            -Vector2.UnitY.RotatedByRandom(MathHelper.PiOver2).RotatedByRandom(.1f) *
                            Main.rand.NextFloat(8f, 50f), Main.rand.NextFloat(.5f, 1.4f), Color.Gold,
                            Color.DarkGoldenrod, Main.rand.NextFloat(200f, 240f));
