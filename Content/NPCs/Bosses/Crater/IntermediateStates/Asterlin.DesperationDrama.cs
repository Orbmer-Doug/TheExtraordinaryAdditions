...
public partial class Asterlin : ModNPC
{
    ...
    public void DoBehavior_DesperationDrama()
    {
        ...
        {
            Dialogue_Manager = new(Vector2.Zero, .9f, .2f);
            Dialogue_Manager.AddSentence(FullDialogue);
        }

        Ominous ??= LoopedSoundManager.CreateNew(
            new(AdditionsSound.PipIdle, () => InverseLerp(TimeToTemp, TimeToHeatsink, AITimer) * .4f),
        ...
        CameraSystem.SetCamera(NPC.Center - Vector2.UnitY * 200f,
            Animators.MakePoly(2.3f).InOutFunction(InverseLerp(0f, DesperationDrama_CameraScrollTime, AITimer)));

        if (AITimer < DesperationDrama_Wait)
        {
            Dialogue_ScreenInterpolant = InverseLerp(DesperationDrama_CameraScrollTime,
                DesperationDrama_CameraScrollTime + DesperationDrama_ScreenPullupTime, AITimer);
        ...
        else
        {
            ...
            {
                Dialogue_ScreenInterpolant =
                    1f - InverseLerp(startToFade, startToFade + DesperationDrama_ScreenPullupTime, AITimer);
                Dialogue_FindingChannel = true;
                this.Sync();
            }
            else
