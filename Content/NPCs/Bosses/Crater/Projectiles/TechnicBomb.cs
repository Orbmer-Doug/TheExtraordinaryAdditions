...
public class TechnicBomb : ProjOwnedByNPC<Asterlin>
{
    ...
    public override void SafeAI()
    {
        ...
        if (Main.rand.NextBool(7))
            ParticleRegistry.SpawnTechyHolosquareParticle(Projectile.Center,
                Main.rand.NextVector2Circular(2f, 2f) + Main.rand.NextVector2Circular(7f, 7f), Main.rand.Next(20, 40),
                Main.rand.NextFloat(.6f, 1.2f), new(18, 193, 255), Main.rand.NextFloat(.7f, 1.2f),
                Main.rand.NextFloat(1f, 1.7f));
        if (Main.rand.NextBool(6))
            ParticleRegistry.SpawnLightningArcParticle(Projectile.Center,
                Main.rand.NextVector2CircularLimited(200f, 200f, .5f, 1f), Main.rand.Next(10, 12),
                Main.rand.NextFloat(.4f, .6f), Color.Cyan);
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            points.Update(Projectile.Center +
                          GetPointOnRotatedEllipse(rad, rad / 3f, Time * .005f, Main.GameUpdateCount * 0.15f + add));
            points2.Update(Projectile.Center + GetPointOnRotatedEllipse(rad, rad / 3f, MathHelper.PiOver2 - Time * .01f,
                Main.GameUpdateCount * 0.15f + MathHelper.Pi + add));
        }
        ...
        {
            if (p.type != ModContent.ProjectileType<TheLightripBullet>() ||
                !p.Hitbox.Intersects(Projectile.Center.ToRectangle(100, 100)) || Hit)
                continue;
            ...
            if (ModProjectile.RunServer())
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    Vector2 vel = (MathHelper.TwoPi * InverseLerp(0f, 3, i) + rand).ToRotationVector2()
                        .RotatedByRandom(.3f) * 3f;
                    SpawnProjectile(Projectile.Center, vel, ModContent.ProjectileType<OverchargedLaser>(),
                        Asterlin.LightAttackDamage, 0f);
                }

                SpawnProjectile(Projectile.Center, Vector2.Zero, ModContent.ProjectileType<TechnicBlast>(),
    ...
    public float WidthFunct(float c) => 40f * MathHelper.SmoothStep(1f, 0f, c) *
                                        GetCircularSectionValue(Main.GameUpdateCount * 0.15f, .3f, .5f, 1f);

    public float WidthFunct2(float c) => 40f * MathHelper.SmoothStep(1f, 0f, c) *
                                         GetCircularSectionValue(Main.GameUpdateCount * 0.15f + MathHelper.Pi, .3f, .5f,
                                             1f, MathHelper.PiOver2);

    public Color ColorFunct(SystemVector2 c, Vector2 pos)
    {
        return new Color(100, 220, 255) * c.X * GetCircularSectionValue(Main.GameUpdateCount * 0.15f, .5f, .8f, 1.8f) *
               Projectile.Opacity;
    }

    public Color ColorFunct2(SystemVector2 c, Vector2 pos)
    {
        return new Color(100, 220, 255) * c.X *
               GetCircularSectionValue(Main.GameUpdateCount * 0.15f + MathHelper.Pi, .5f, .8f, 1.8f,
    ...
    public override bool PreDraw(ref Color lightColor)
    {
        ...
        void glow()
        {
            ...
            Main.spriteBatch.DrawBetterRect(tex, ToTarget(Projectile.Center, size * .5f), null,
                col.Lerp(Color.White, .6f), 0f, tex.Size() / 2f);
            Main.spriteBatch.DrawBetterRect(tex, ToTarget(Projectile.Center, size * .68f), null,
                col.Lerp(Color.White, .3f) * .9f, 0f, tex.Size() / 2f);
            Main.spriteBatch.DrawBetterRect(tex, ToTarget(Projectile.Center, size * .82f), null, col * .8f, 0f,
                tex.Size() / 2f);
        ...
        if (Time == 0)
        {
            for (int i = 0; i < 50; i++)
                ParticleRegistry.SpawnBloomLineParticle(Projectile.Center,
                    Main.rand.NextVector2Circular(5f, 5f) + Main.rand.NextVector2CircularLimited(10f, 10f, .5f, 1f),
                    Main.rand.Next(20, 30), Main.rand.NextFloat(.3f, .5f), new(112, 218, 255));

            for (int i = 0; i < 20; i++)
                ParticleRegistry.SpawnLightningArcParticle(Projectile.Center,
                    Main.rand.NextVector2CircularLimited(120f, 120f, .7f, 1f), 12, Main.rand.NextFloat(.2f, .3f),
                    new(66, 206, 255));

            for (int i = 0; i < 30; i++)
                ParticleRegistry.SpawnSparkParticle(Projectile.Center, Main.rand.NextVector2Circular(1f, 1f),
                    Main.rand.Next(30, 40), Main.rand.NextFloat(.9f, 1.3f), Color.Cyan);
        }
