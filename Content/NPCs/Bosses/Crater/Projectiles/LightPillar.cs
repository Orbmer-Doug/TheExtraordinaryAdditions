        ...
        if (!HitGround)
        {
            ...
            for (int i = 0; i < 30; i++)
            {
                Vector2 pos = Projectile.Center + Vector2.UnitY * Projectile.width / 2 +
                              Vector2.UnitX * Main.rand.NextFloat(-Projectile.width / 2, Projectile.width / 2);
                ParticleRegistry.SpawnBloomLineParticle(pos, -Vector2.UnitY * Main.rand.NextFloat(1f, 9f),
                    Main.rand.Next(30, 50), Main.rand.NextFloat(.4f, .8f), Color.OrangeRed);
                ParticleRegistry.SpawnSquishyPixelParticle(pos,
                    -Vector2.UnitY.RotatedByRandom(.1f) * Main.rand.NextFloat(3f, 14f), Main.rand.Next(30, 60),
                    Main.rand.NextFloat(1.8f, 2.9f), Color.OrangeRed, Color.Gold, 5);
