...
namespace TheExtraordinaryAdditions.Content.NPCs.Bosses.EXAMPLE;
...
public class MinionBossBody : ModNPC
{
    ...
    private void DoSecondStage_SpawnEyes(Player player)
    {
        ...
        if (NPC.HasValidTarget && SecondStageTimer_SpawnEyes == 0 && Main.netMode != NetmodeID.MultiplayerClient)
        {
            ...
            NPC.NewNPCProj(position, -Vector2.UnitY, type, damage, 0f);
        }
    }
...
public class MinionBossEye : ProjOwnedByNPC<MinionBossBody>
{
    public override void SafeAI()
    {
        if (Projectile.ai[2] == 0f)
        {
            ($"\nsup im bosseye at index {Projectile.identity} and I can use Owner: {Owner}, Boss: {Boss} and Target: {Target}").ServerLog();
            Projectile.ai[2] = 1f;
