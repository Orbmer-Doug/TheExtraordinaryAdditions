...
public class BlueTopHat : ModItem
{
    ...
    public override void SetStaticDefaults()
    {
        ...
        ArmorIDs.Head.Sets.DrawHatHair[Item.headSlot] =
            true; // Draw hair as if a hat was covering the top. Used by Wizards Hat
...
//TODO
        //if (AdditionsKeybinds.SetBonusHotKey.JustPressed && !.HasCooldown(player, MyceliumiteCooldown.ID) &&
        //  player.whoAmI == Main.myPlayer)
        {
            ...
            for (int i = 0; i < amt; i++)
            {
                ...
                Vector2 position = new Vector2(player.position.X - away, player.position.Y + player.height + num) +
                                   Vector2.UnitY.RotatedBy(player.fullRotation);
                ...
                for (int j = 0; j < 5; j++)
                {
                    if (j % 2f == 1f)
                    {
                        ParticleRegistry.SpawnMistParticle(shroomPos, RandomVelocity(2f, 3f, 4f),
                            Main.rand.NextFloat(.5f, .8f), new(95, 110, 225), new(31, 32, 110),
                            Main.rand.NextByte(120, 200), .05f);
                    }

                    Dust.NewDustPerfect(shroomPos, DustID.GlowingMushroom,
                        vel.RotatedByRandom(.1f) * Main.rand.NextFloat(.2f, .4f), 0, default,
            ...
            //TODO
            //    .AddCooldown(player, MyceliumiteCooldown.ID, SecondsToFrames(15));
        }
    }
