...
public class AncientBoon : ModItem
{
    ...
    public override void UpdateAccessory(Player player, bool hideVisual)
    {
        ...
        if (!player.slowFall && player.wingTime < player.wingTimeMax && !player.controlJump &&
            player.miscCounter % 2 == 0)
            ...
            if (info.Damage > 0)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    AncientRetaliation proj = Main.projectile[Player.NewPlayerProj(Player.MountedCenter, vel,
                            ModContent.ProjectileType<AncientRetaliation>(), dmg, 1f, Player.whoAmI)]
                        .As<AncientRetaliation>();
                    proj.Direction = dir;
                }
            }
        }
