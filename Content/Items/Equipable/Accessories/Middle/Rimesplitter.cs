...
public class Rimesplitter : ModItem, ILocalizedModType, IModType
{
    ...
    public override void UpdateAccessory(Player player, bool hideVisual)
    {
            ...
            player.whoAmI == Main.myPlayer)
        {
            ...
            if (player.CountOwnerProjectiles(type) <= 0 && !player.HasBuff(ModContent.BuffType<AuroricCooldown>()))
            {
                int dmg = (int) player.GetTotalDamage(DamageClass.Generic).ApplyTo(500f);
                Projectile p =
                    Main.projectile[
                        Projectile.NewProjectile(player.GetSource_ItemUse(Item, null), player.Center, Vector2.One, type,
                            dmg, 10f, player.whoAmI, 0f, 0f, 0f)];
