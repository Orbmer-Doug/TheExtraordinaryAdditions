static const float resolution = 256 * 3;
...
float3 borderColor = float3(1, .2, .2);
float edgeDetect(float2 uv, float2 res)
{
    float step = 1.0 / res; // Step size based on resolution
    float h = tex2D(Target, uv + float2(step, 0)).r - tex2D(Target, uv - float2(step, 0)).r;
    float v = tex2D(Target, uv + float2(0, step)).r - tex2D(Target, uv - float2(0, step)).r;
    return sqrt(h * h + v * v) * 1.5; // Magnitude of gradient
}
...
float4 PixelShaderFunction(float4 sampleColor : COLOR0, float2 coords : TEXCOORD0) : COLOR0
{
    ...
    lavaColor = pow(abs(lavaColor), 1.5);
    ...
    lavaColor += glowColor * pow(bright, 2.0) * .2;
    
    float influence = tex2D(Target, coords).r;
    float alpha = smoothstep(threshold - epsilon, threshold + epsilon, influence);

    float4 final = float4(lavaColor, alpha);
    
    float4 meta = float4(tex2D(Target, coords).rgb * (alpha * 2), alpha);
    final *= meta;
    
    float edge = edgeDetect(coords, (resolution * 1.5) * alpha);
    float3 border = (float3(1, 0.2, 0.05)) * edge * (6.0 - alpha);
    
    return floor((lerp(final, float4(border, 1), edge) * 2) * quantization) / quantization; //(floor(float4(lavaColor, alpha) * quantization) / quantization) * (alpha * 2);
}
